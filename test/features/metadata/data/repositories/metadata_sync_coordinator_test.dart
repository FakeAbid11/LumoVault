...
void main() {
  group('MetadataSyncCoordinator', () {
    ...
    tearDown(() async {
      // Stop the listener first, then drain whatever it already started —
      // otherwise the temp directory goes away under a live upload.
      coordinator.dispose();
    ...
    ///
    /// The auto-sync path is a debounce plus a temp-file write and an upload
    /// per document, fired from a stream listener no caller can await. A fixed
    /// sleep has to be either long enough to slow every run or short enough to
    /// lose the race on a loaded CI machine; polling is neither, and returning
    /// on timeout lets the assertion that follows produce the real diagnosis.
    ...
    test('gallery mutations auto-sync after the debounced flush', () async {
      ...
      await waitUntil(() => uploadedFiles().length >= 2);
