  ...
  group('StorageChannelService', () {
    ...
    group('findOrCreateChannel', () {
      ...
      test('returns error on search failure', () async {
        ...
        // A failed search must NOT fall through to creation: we don't know
        // whether a channel already exists, and creating one here is how a
        // user's backup ends up split across two channels.
        expect(result, isA<StorageChannelError>());
    ...
    group('channel creation', () {
      ...
      test('pins manifest message after creation', () async {
        ...
        // Asserted against the live constant rather than a literal: the pinned
        // record used to hardcode schema_version 1, so every restore's
        // compatibility check passed against a constant instead of against the
        // version that actually wrote the data.
        expect(
          text,
          contains('"schema_version":${Manifest.currentSchemaVersion}'),
        );
        final pinned = Manifest.fromJsonString(text);
        expect(pinned?.isCompatibleWith(Manifest.currentSchemaVersion), isTrue);
      });
    });
  });
