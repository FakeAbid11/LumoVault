...
import '../../people/data/services/face_detection_service.dart';
...
class BackgroundBackupService implements BackupTaskScheduler {
  ...
  Future<void> registerAllTasks({
    ...
    // No registerFaceScan() here. Face/AI/CLIP scheduling is owned by
    // autoScanSyncProvider, which registers or cancels from AppSettings flags
    // that BackupSettings doesn't carry — registering unconditionally on this
    // path re-enabled the periodic face scan behind the user's back every time
    // any backup setting changed, even with face scanning switched off.
  ...
  Future<void> registerClipEmbedding() async {
    ...
    debugPrint(
      '[BackgroundBackupService] Registered CLIP embedding generation',
    );
  }
...
class BackgroundTaskRunner {
  ...
  Future<bool> _handleMediaScanner() {
    // Runs under the backup lock. scanAndEnqueue persists a whole-queue JSON
    // snapshot, and this used to take no lock at all — so a 15-minute scanner
    // firing while an upload worker was mid-batch overwrote the worker's state
    // with the scanner's stale view, dropping in-flight tasks.
    return _withBackupLock((container) async {
      ...
      // Live queue count, not engine.stats: stats is throttled ~500ms behind
      // the scan that just filled the queue, so this read it as 0 and never
      // asked for the upload run.
      final pending = engine.queuedPendingCount;
  ...
  Future<bool> _handleMetadataRepair() async {
    return _withContainer((container) async {
      ...
      // Reports only: the fixable issues (empty hash/MIME, missing thumbnail)
      // self-correct on the next scan or thumbnail rebuild, so there is no
      // repair step to run here — only an accurate count to log.
      ...
      debugPrint(
        ...
        'issue(s), ${result.fixableCount} will self-correct on next scan',
      );
      return true;
    });
  }

  /// Trim the derived-image caches back under their size budgets.
  ///
  /// Two separate stores: the bounded ThumbnailCache directory, and the
  /// per-face crops that live in the OS temp root referenced by absolute path
  /// from the database (see [evictFaceCropCache]) and so are invisible to
  /// ThumbnailCache's own eviction.
  Future<bool> _handleThumbnailRebuild() async {
    final cache = ThumbnailCache.instance;
    await cache.initialize();
    await cache.evictIfOverSize();
    final cropsRemoved = await evictFaceCropCache();
    debugPrint(
      '[BackgroundBackup] Thumbnail cache now '
      '${await cache.getDiskCacheCount()} entr(ies), '
      '$cropsRemoved face crop(s) trimmed',
    );
    return true;
  }
  ...
  Future<bool> _handleFaceScan() async {
    ...
    if (!await lock.tryAcquire()) {
      debugPrint(
        '[BackgroundBackup] Face scan: another scan is running, skipping',
      );
      return true;
    }
    try {
      return await _withContainer((container) async {
        try {
          ...
          // Mirror FaceScanController.start() on the UI isolate: hidden and
          // trashed photos must never be face-scanned in the background
          // either — no crops materialized, no embeddings stored for the
          // private vault or pending deletions.
          final gallery = container.read(galleryRepositoryProvider);
          await gallery.hydrate();
          final toScan = assets
              .where((a) {
              ...
              .where((a) => !scannedIds.contains(a.id))
              .toList();
          if (toScan.isEmpty) {
          ...
          debugPrint(
            '[BackgroundBackup] Face scan: ${toScan.length} new photos',
          );
  ...
  Future<bool> _runBackup(
    ...
    try {
      // Hydrate UNCONDITIONALLY. This used to sit inside `if (scan)`, so the
      // upload worker — which runs with scan: false — drove the engine against
      // an empty in-memory gallery. markUploaded returns silently when the
      // localId isn't in that index, so the file landed in Telegram while the
      // database stayed `pending`: every subsequent scan re-enqueued and
      // re-uploaded the same photo as a channel duplicate, forever.
      await container.read(galleryRepositoryProvider).hydrate();
      // Queue restore is kicked off from the engine's constructor and can't be
      // awaited there, so read the live queue only after it has settled.
      await engine.ready;

      if (scan) await engine.scanAndEnqueue();

      // queuedPendingCount, not stats.pendingCount: stats is throttled and was
      // last refreshed 500ms after the scan, so this gate routinely saw 0 and
      // ended the run with "Nothing queued, finishing" while real work waited.
      if (engine.queuedPendingCount == 0) {
