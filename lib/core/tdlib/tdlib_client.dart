...
class TdLibClient {
  ...
  Future<void> initialize({required String databaseKey}) async {
    ...
    try {
      await _bootstrapCompleter!.future.timeout(
        ...
        onTimeout: () {
          debugPrint('[TdLibClient] Bootstrap timed out — TDLib may be stuck');
        },
      );
    } finally {
  ...
  Future<Map<String, dynamic>> sendRequest({
    ...
    try {
      await _sendJson(jsonEncode(request));
    } catch (e) {
      // A request that never left the process will never get a response, so
      // without this the entry and its Completer stayed in
      // _requestCompleters forever — the .timeout below only protects sends
      // that actually got away. It leaked one pair per failed send for the
  ...
  Future<void> close() async {
    ...
    if (clientId != null) {
      // Asks TDLib itself to shut the session down; still needs the receive
      // loop alive to deliver the response.
      try {
    ...
    // Stop the receive isolate BEFORE the native handle is freed. That loop
    // blocks inside `tdJsonClientReceive(clientId, 1.0)`; destroying the client
    // first left it calling into freed memory — a use-after-free that surfaced
    // as a native SIGSEGV on logout/reconnect in release builds.
