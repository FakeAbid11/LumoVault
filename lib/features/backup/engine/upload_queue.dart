...
class UploadQueue {
  ...
  ///
  /// An explicit user action, so the automatic retry budget is *reset* rather
  /// than consulted. This used to skip tasks where `canRetry` was false and
  /// increment the rest — but every failure already consumes an attempt in
  /// [BackupEngine], so once a task burned its budget the dashboard's "Retry
  /// Failed" button kept rendering while doing nothing at all, and the item
  /// could never be uploaded without reinstalling.
  void retryAllFailed() {
  ...
  /// Retry a single failed task (reset to queued), giving it a fresh attempt
  /// budget for the same reason [retryAllFailed] does.
  ///
  /// No-op if the task isn't in the queue or isn't failed.
