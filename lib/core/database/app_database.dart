...
class Faces extends Table {
  ...
  TextColumn get embeddingModel => text().withDefault(const Constant(''))();
...
class People extends Table {
  ...
  TextColumn get centroidModel => text().withDefault(const Constant(''))();
}
...
class AppDatabase extends _$AppDatabase {
  ...
  MigrationStrategy get migration => MigrationStrategy(
    ...
    onUpgrade: (m, from, to) async {
      ...
      if (from < 20 && to >= 20) {
        // Provenance columns + a clean People rebuild. Before v20 nothing
        // recorded WHICH embedder produced a vector, and the silent
        // EdgeFace->w600k fallback let faces from two non-comparable
        // embedding spaces merge people across models (the shared-prefix
        // cosine never refused them). Existing vectors therefore have
        // unverifiable provenance: wipe the face graph and rebuild it
        // model-tagged under the hardened rules. Names go with the faces
        // they were assigned to (same precedent as v10/v11/v12/v18);
        // same-era rescans keep named anchors from here on.
        //
        // An ADD-COLUMN helper guarded against duplicates: tables rebuilt by
        // m.createTable() earlier in an upgrade chain already carry the
        // current schema, and SQLite has no `ADD COLUMN IF NOT EXISTS`.
        Future<void> addColumn(
          ...
          try {
            ...
            // duplicate column — the current schema was already created
            // mid-chain; nothing to add.
          }
        }

        await addColumn('faces', 'embedding_model', "TEXT NOT NULL DEFAULT ''");
        await addColumn('faces', 'excluded', 'INTEGER NOT NULL DEFAULT 0');
        await addColumn('people', 'centroid_model', "TEXT NOT NULL DEFAULT ''");
        await m.database.customStatement('DELETE FROM face_persons');
        await m.database.customStatement('DELETE FROM faces');
        await m.database.customStatement('DELETE FROM people');
        await m.database.customStatement('DELETE FROM face_scans');
