  ...
  void _handleUploadUpdate({
    ...
    switch (type) {
      ...
      case 'updateMessageSendFailed':
        ...
        // Derive the semantic code the same way TdLibException.fromResponse
        // does: error['code'] is only the HTTP-style numeric status (429),
        // while the semantic identifier ("FLOOD_WAIT_30") is error['message'].
        // Passing the raw numeric code here used to make every flood wait
        // classify as 'unknown' and fail the task permanently.
        final tdException = error != null
            ? TdLibErrorMapper.fromResponse(error)
            : const TdLibException(message: 'Upload failed', code: 'UNKNOWN');
        if (!completer.isCompleted) {
          completer.completeError(
            TransferError.fromTdLibError(tdException.code, tdException.message),
          );
        }
    }
  }
