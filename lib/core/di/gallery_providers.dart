...
final galleryRepositoryProvider = Provider<GalleryRepository>((ref) {
  ...
  // Bump the version counter on metadata mutations so providers that watch
  // [galleryDataVersionProvider] (searchProvider, filteredSortedAssetsProvider,
  // the timeline) re-evaluate.
  //
  // Coalesced with a microtask: the repository fires this callback on *every*
  // single write, so a bulk operation — a multi-select move-to-trash, a
  // person-name reload, mergeTelegramItems after a restore — used to queue one
  // invalidation per item, and each one re-ran every O(library) consumer below
  // it. Consumers only ever ask "did anything change", never "how many times".
  //
  // NOTE this only merges mutations landing in the same synchronous block;
  // writes separated by an await (a background AI scan, say) still bump one at
  // a time, so callers must not assume a single invalidation per operation.
  var bumpQueued = false;
  var containerLive = true;
  ref.onDispose(() => containerLive = false);
  repository.onDataChanged = () {
    if (bumpQueued || !containerLive) return;
    bumpQueued = true;
    scheduleMicrotask(() {
      bumpQueued = false;
...
final filteredSortedAssetsProvider = Provider<List<AssetEntity>>((ref) {
  ...
  final repository = ref.watch(galleryRepositoryProvider);
  // This reads hidden/trashed/favorite/tag state out of [repository] below, so
  // it has to recompute when that state changes. Without the watch the Local
  // tab kept showing a photo the user had just hidden for the rest of the
  // session, and the Favorites filter lagged every toggle.
...
final duplicateGroupsProvider = Provider<List<List<MediaItem>>>((ref) {
  final repository = ref.watch(galleryRepositoryProvider);
