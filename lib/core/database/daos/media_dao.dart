...
class MediaDao extends DatabaseAccessor<AppDatabase> with _$MediaDaoMixin {
  ...
  /// Items belonging to a device folder / album, newest first. Excludes
  /// trashed and hidden items, matching the timeline semantics.
  ...
  /// Favorited items, newest first. Excludes trashed and hidden items.
  ...
  /// Case-insensitive search over file name and description. Excludes
  /// trashed and hidden items.
  ///
  /// Note the query is bound as a parameter, so this is not an injection risk —
  /// but it does mean LIKE's own wildcards are live in user text: searching for
  /// `%` matches everything and `_` matches any single character. Suppressing
  /// them needs a `LIKE ? ESCAPE '\'` clause, which drift's `.like()` cannot
  /// emit and `CustomExpression` cannot supply a bound variable for; hand-
  /// inlining the pattern would trade a cosmetic search quirk for a real SQL
  /// injection hole. Left as-is on purpose.
  ...
  /// Batch lookup by stable platform media id, keyed by localId.
  ///
  /// Used to carry existing database primary keys forward onto freshly
  /// scanned items before an upsert, so `ON CONFLICT` targets the right row
  /// instead of colliding with the `localId` unique constraint.
  Future<Map<String, MediaItemRow>> byLocalIds(List<String> localIds) async {
    if (localIds.isEmpty) return const {};
    final rows = await (select(
      mediaItems,
    )..where((t) => t.localId.isIn(localIds))).get();
    return {for (final row in rows) row.localId: row};
  }
