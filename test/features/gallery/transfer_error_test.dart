...
void main() {
  group('TransferError', () {
    ...
    test('fromTdLibError maps FILE_TOO_BIG and STORAGE_FULL separately', () {
      final tooBig = TransferError.fromTdLibError(
        'FILE_TOO_BIG',
        'File is too big',
      );
      final storageFull = TransferError.fromTdLibError(
        'STORAGE_FULL',
        'No space',
      );

      expect(tooBig.category, TransferErrorCategory.fileTooLarge);
      expect(tooBig.retryable, false);
      expect(tooBig.displayMessage, contains('2GB'));

      expect(storageFull.category, TransferErrorCategory.storageFull);
      expect(storageFull.retryable, false);
      expect(storageFull.displayMessage, contains('Telegram storage'));
    });
    ...
    // Regression: TDLib's semantic flood-wait code carries a per-wait suffix
    // ("FLOOD_WAIT_30"). The switch used to match the bare literal 'FLOOD_WAIT'
    // and never fired, so a rate-limited upload classified as unknown with
    // retryable: false and was permanently failed on its FIRST flood wait.
    test('classifies suffixed FLOOD_WAIT codes with retryAfter from code', () {
      final error = TransferError.fromTdLibError(
        'FLOOD_WAIT_30',
        'Too Many Requests: retry after 30',
      );

      expect(error.category, TransferErrorCategory.floodWait);
      expect(error.retryable, true);
      expect(error.retryAfterSeconds, 30);
      expect(error.displayMessage, contains('30'));
    });

    test('extracts retryAfter from message when the code has no number', () {
      final error = TransferError.fromTdLibError(
        'FLOOD_WAIT',
    ...
    test('numeric HTTP code alone does not classify as flood wait', () {
      // The raw numeric status (429) carries no semantic meaning — only the
      // message field does — so it must fall through to unknown rather than
      // be mistaken for a flood wait.
      final error = TransferError.fromTdLibError('429', 'Too Many Requests');

      expect(error.category, TransferErrorCategory.unknown);
