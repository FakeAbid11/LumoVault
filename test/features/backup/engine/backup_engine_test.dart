...
void main() {
  ...
  group('BackupEngine retry budget and persistence', () {
    test(
      'a retryable failure gets exactly maxAttempts transfers, not one more',
      () async {
        // retryAfterSeconds pins the backoff to its 1s clamp floor — the real
        // 5s exponential base would make this a 15s test for three attempts.
        final repo = _repo([_item('a')]);
        final uploads = _FakeUploadService()
          ..error = TransferError(
            category: TransferErrorCategory.unknown,
        ...
        // Two 1s backoffs elapse before the third attempt; poll instead of
        // sleeping a fixed interval so a faster machine doesn't wait the full
        // budget every run.
        for (
          var i = 0;
          i < 60 && uploads.attempts < UploadTask.maxAttempts;
          i++
        ) {
          await Future<void>.delayed(const Duration(milliseconds: 100));
        }

        // The guard used to read `retryCount < maxAttempts` against the
        // pre-increment count, so retryCount 0,1,2 and 3 all passed — four
        // transfers for a maxAttempts of three.
        expect(uploads.attempts, UploadTask.maxAttempts);
        expect(engine.queue.failedCount, 1);
      },
    );

    test(
      'a task cancelled mid-flight is not resurrected by its own retry',
      () async {
        final repo = _repo([_item('a')]);
        final uploads = _FakeUploadService()
          ..error = TransferError(
        ...
        // Remove the task the instant its upload starts, before the retryable
        // failure handler runs. The retry path used to requeue via
        // _queue.updateTask with a `?? task` fallback, and updateTask inserts
        // unknown ids by design — so the cancelled task came back and ran
        // anyway.
        uploads.onUpload = (task) => engine.cancelTask(task.id);

        engine.addToQueue(repo.mediaItems.single);
        await engine.startBackup();

        // Let the deferred retry wake fire: there must be nothing to resurrect.
        await Future<void>.delayed(const Duration(milliseconds: 1200));

        expect(uploads.attempts, 1);
        expect(engine.queue.getTaskForMediaItem('a'), isNull);
        expect(engine.queue.allTasks, isEmpty);
      },
    );

    test(
      'dispose flushes an armed persist instead of dropping the queue',
      () async {
        final repo = _repo([_item('a')]);
        final persistence = _RecordingPersistence();
        final engine = _engine(repo: repo, persistence: persistence);
        // No addTearDown(engine.dispose): this test disposes it explicitly.

        engine.addToQueue(repo.mediaItems.single);
        // Let the throttled stats callback arm the 5s persist timer.
        await Future<void>.delayed(const Duration(milliseconds: 20));

        // Teardown cancels that timer; without the flush, a queue the scanner
        // task populated moments earlier would never reach disk.
        engine.dispose();

        expect(persistence.saved, [
          predicate<List<UploadTask>>(
            (tasks) => tasks.length == 1 && tasks.single.fileName == 'a.jpg',
          ),
        ]);
      },
    );
...
/// Records [TransferQueuePersistence] saves so the dispose-flush path can
/// assert the queue reached disk without touching a real file.
class _RecordingPersistence implements TransferQueuePersistence {
  final List<List<UploadTask>> saved = [];
  ...
  Future<void> saveTasks(List<UploadTask> tasks) async {
    saved.add(List<UploadTask>.of(tasks));
  }

  @override
  Future<List<UploadTask>> loadTasks() async => const [];
  ...
  @override
  Map<String, dynamic> encodeTask(UploadTask task) =>
      throw UnimplementedError();

  @override
  UploadTask decodeTask(Map<String, dynamic> json) =>
      throw UnimplementedError();
}
