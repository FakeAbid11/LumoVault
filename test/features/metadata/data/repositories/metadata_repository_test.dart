...
void main() {
  group('MetadataRepository', () {
    ...
    test('recordDeletion tombstones the item on permanent delete', () {
      final item = MediaItem(
        localId: '123',
        fileHash: 'abc123',
        filePath: '/path/file.jpg',
      ...
      // A permanent delete keeps the item as a tombstone so the deletion
      // re-syncs, survives a restore, and converges other devices.
      final metadata = repository.getItemMetadata('123');
      expect(metadata, isNotNull);
      expect(metadata!.isDeleted, isTrue);
      expect(metadata.deletedAt, isNotNull);
    });

    test('recordDeletion removes the item outright on scan_delete', () {
      final item = MediaItem(
        localId: '123',
        fileHash: 'abc123',
        filePath: '/path/file.jpg',
      ...
      // A scan noticing the file left this device removes it locally only —
      // no tombstone, so the channel backup on other devices is untouched.
      repository.recordDeletion(localId: '123', operation: 'scan_delete');
