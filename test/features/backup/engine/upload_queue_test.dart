...
void main() {
  ...
  group('UploadQueue', () {
    ...
    setUp(() {
      queue = UploadQueue(batchSize: 10);
    });
    ...
    test('retryTask requeues a failed task with a fresh attempt', () {
      final item = MediaItem(
        localId: '1',
        fileHash: 'hash1',
        filePath: '/path/test.jpg',
      ...
      final updated = queue.getTaskById(task.id)!;
      expect(updated.status, UploadStatus.queued);
      // An explicit user retry grants a clean slate: the automatic budget is
      // reset rather than decremented further, so a task that failed five
      // times on its own is still retryable when the user asks for it.
    ...
    test('retryTask is a no-op for non-failed tasks', () {
      final item = MediaItem(
        localId: '1',
        fileHash: 'hash1',
        filePath: '/path/test.jpg',
      ...
      expect(queue.getTaskById(task.id)!.status, UploadStatus.queued);
      expect(queue.getTaskById(task.id)!.retryCount, 0);
    });

    test('retryTask revives a task that exhausted its automatic budget', () {
      final item = MediaItem(
        localId: '1',
        fileHash: 'hash1',
        filePath: '/path/test.jpg',
      ...
      queue.updateTask(
        task.copyWith(
          status: UploadStatus.failed,
          retryCount: UploadTask.maxAttempts,
        ),
      );
      ...
      final updated = queue.getTaskById(task.id)!;
      // Retrying on explicit request must NOT be gated on the automatic
      // budget: it used to be, so once a task burned all its attempts the
      // dashboard's "Retry Failed" button kept rendering while silently
      // refusing to ever upload the item again.
      expect(updated.status, UploadStatus.queued);
      expect(updated.retryCount, 0);
      expect(queue.failedCount, 0);
      expect(queue.pendingCount, 1);
    ...
    test('retryTask is a no-op for unknown task ids', () {
      queue.retryTask('does-not-exist');
    ...
    test('updateTask with a changed priority leaves no stale duplicate', () {
      // The queue is a SplayTreeSet ordered by (priority, id), so a task's
      // position moves when its priority changes. updateTask has to remove the
      // *old* object — removing by the updated one would fail to locate it and
      // leave both copies in the queue, double-counting the upload forever.
      final item = MediaItem(
        localId: '1',
        fileHash: 'hash1',
        filePath: '/path/test.jpg',
      ...
      final task = queue.enqueue(item: item)!;
      queue.updateTask(task.copyWith(priority: task.priority + 500));

      expect(queue.totalCount, 1);
      expect(queue.getTaskById(task.id)!.priority, task.priority + 500);
    });

    test('removeByMediaItem frees the item to be enqueued again', () {
      final item = MediaItem(
        localId: '1',
        fileHash: 'hash1',
        filePath: '/path/test.jpg',
      ...
      queue.removeByMediaItem('1');

      expect(queue.totalCount, 0);
      expect(queue.hasTaskForMediaItem('1'), isFalse);
      // The duplicate-check index is secondary state. If a removal doesn't
      // unwind it, the media item becomes permanently un-enqueueable — the
      // user de-selects a photo, re-selects it, and nothing happens.
      expect(queue.enqueue(item: item), isNotNull);
    });

    test('clearFinished releases both the media-item and hash indexes', () {
      final item = MediaItem(
        localId: '1',
        fileHash: 'hash1',
        filePath: '/path/test.jpg',
      ...
      final task = queue.enqueue(item: item)!;
      queue.updateTask(task.copyWith(status: UploadStatus.completed));
      expect(queue.isAlreadyBackedUp('hash1'), isTrue);

      queue.clearFinished();

      expect(queue.isAlreadyBackedUp('hash1'), isFalse);
      expect(queue.hasTaskForMediaItem('1'), isFalse);
    });
  });
}
