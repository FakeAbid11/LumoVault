...
class _MapScreenState extends ConsumerState<MapScreen> {
  ...
  Widget _buildBody(BuildContext context, List<MediaItem> photos) {
    // Defensive: mapPhotosProvider filters for hasLocation, but the unwraps
    // below (latitude!/longitude!) and points.first all assume non-null
    // coordinates. A provider change or a stale cache entry that slips an
    // unlocated item through must not crash the map.
    ...
    final points = [for (final p in located) LatLng(p.latitude!, p.longitude!)];
    ...
    return FlutterMap(
      ...
      children: [
        const OsmTileLayer(),
        _buildClusterLayer(context, located),
        RichAttributionWidget(
          attributions: [
            TextSourceAttribution(
              'OpenStreetMap contributors',
