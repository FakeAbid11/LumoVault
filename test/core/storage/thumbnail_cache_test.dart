...
void main() {
  group('ThumbnailCache', () {
    ...
    test('put does not throw and is retrievable before initialize', () async {
      // Regression: put() used to crash on a null cache directory when the
      // app wrote a thumbnail before initialize() had run. It must now
      // degrade to memory-only storage.
      await cache.put('pre-init', Uint8List.fromList([1, 2, 3]));
      final result = await cache.get('pre-init');
      expect(result, isNotNull);
      expect(result!.length, equals(3));
      await cache.remove('pre-init');
    });
