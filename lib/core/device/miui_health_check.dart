...
class MiuiHealthCheck {
  ...
  static Widget buildWarningBanner({
    ...
    return MaterialBanner(
      ...
      actions: [
        TextButton(
          // Awaited + reported: a fire-and-forget result here used to leave
          // the user with NO feedback when the launch failed.
          onPressed: () async {
            final opened = await BrandSettings.openBatterySettings(packageName);
            if (!opened && context.mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text(kSettingsLaunchFallbackHint)),
              );
            }
          },
