...
class Manifest {
  ...
  /// - 1: original push-only layout (no deletion tombstones).
  /// - 2: adds per-item deletion tombstones (isDeleted/deletedAt) for two-way
  ///   sync. Backward compatible — v1 partitions simply carry no tombstones,
  ///   so [isCompatibleWith] still accepts them.
  ...
  static Manifest? fromJsonString(String jsonString) {
    try {
      ...
      debugPrint('[Manifest] Failed to parse manifest JSON: $e');
      return null;
    }
  }
