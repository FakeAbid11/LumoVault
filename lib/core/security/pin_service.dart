...
class PinService {
  PinService({this.iterations = AppConstants.pbkdf2Iterations, Random? random})
  ...
  String hashPin(String pin) {
    ...
    final digest = _pbkdf2Sync(pin: pin, salt: salt, iterations: iterations);
  ...
  /// PBKDF2-HMAC-SHA256 (RFC 8018) — synchronous delegate.
  ///
  /// Used by [hashPin] on the main isolate (salt generation + hash in one
  /// call). For [verifyPin], the top-level [_pbkdf2] runs on a background
  /// isolate instead.
...
/// Top-level PBKDF2 function that can be sent to an [Isolate].
///
/// Must be a top-level or static function (not a closure or instance method)
/// so Dart's isolate boundary can serialize it.
Uint8List _pbkdf2({
  required String pin,
  required Uint8List salt,
  required int iterations,
  int keyBytes = PinService._keyBytes,
}) {
