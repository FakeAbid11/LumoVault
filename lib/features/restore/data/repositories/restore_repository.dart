...
import '../../../metadata/data/repositories/telegram_metadata_downloader.dart';
...
class RestoreRepository {
  ...
  /// Fetch the backup's manifest, preferring the live manifest *document*.
  ///
  /// The pinned message used to be the only source read here, and it is a
  /// frozen stub: [_pinManifestMessage] writes it exactly once at channel
  /// creation with `total_media: 0`, an empty chunk list and a hardcoded
  /// `schema_version: 1`, and nothing ever updates it — every later manifest
  /// change goes to the document that [TelegramMetadataUploader] writes. So a
  /// restore reported an empty backup, its ETA was always zero, and the
  /// schema-compatibility check compared against a constant instead of against
  /// the version that actually produced the data.
  ///
  /// The pinned message remains as a fallback so a backup created before the
  /// document existed is still restorable.
  Future<Manifest?> fetchManifest(int channelId) async {
    final downloader = TelegramMetadataDownloader(
      client: _client,
      downloadService: _downloadService,
      storageChannelService: _storageChannelService,
      // Pin the pass to the channel restore already resolved, rather than
      // letting the downloader pick a channel of its own.
      channelIdProvider: () async => channelId,
    );
    final document = await downloader.downloadManifest();
    if (document != null) return document;

    return _fetchPinnedManifest(channelId);
  }

  /// Fetch the legacy manifest from the channel's pinned message.
  ///
  ...
  Future<Manifest?> _fetchPinnedManifest(int channelId) async {
