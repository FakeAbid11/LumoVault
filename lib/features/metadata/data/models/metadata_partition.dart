...
class MetadataPartition {
  ...
  /// name, tags, the user-set-date flag, the user-set location name, and AI
  /// labels. Leaving any of those out (as an earlier version did) meant an
  /// edit to e.g. an item's album or tags never changed the hash — the
  /// partition never looked dirty, and the change was never uploaded to
  /// Telegram.
  ///
  /// `locationName` and `aiLabels` were the last two holdouts: a location or
  /// label edit left the digest unchanged, so the change was silently dropped
  /// on every push, and [ConflictResolver.areIdentical] saw a remote edit as
  /// "no conflict" on every pull. `isDateUserSet` was the next one: an EXIF
  /// date edit flipped the flag and moved the item between partitions, but
  /// the flag itself never reached the digest, so the "date is user-set"
  /// half of the edit did not propagate.
  ///
        ...
        items
            .map(
              (item) => [
                ...
                item.locationName ?? '',
                (item.aiLabels.toList()..sort()).join(','),
