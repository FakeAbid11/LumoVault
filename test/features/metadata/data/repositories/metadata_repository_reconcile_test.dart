...
void main() {
  group('MetadataRepository.reconcileFromTelegram', () {
    ...
    test('applies a remote manifest from an older schema version', () async {
      await repository.recordNewItem(mediaItem('a'));

      // A backup made by an older app version carries schemaVersion 1. v2 was
      // additive (tombstones default on read), so the manifest stays valid and
      // reconcile must accept it rather than refuse it as unmigratable.
      final remoteItem = PartitionItem(
        localId: 'b',
        fileHash: 'hash_b',
      ...
      final remoteManifest = Manifest.create(deviceHash: 'device').copyWith(
        schemaVersion: 1,
        chunks: [
          ManifestChunk(
            id: '2026/02',
      ...
      expect(applied, 1);
      expect(repository.getItemMetadata('b'), isNotNull);
      // The baseline advances, and is stored at the current schema version
      // rather than the older one that came off the wire.
