...
class _VideoThumbnailScrubberState extends State<VideoThumbnailScrubber> {
  ...
  Future<void> _generateThumbnails() async {
    ...
    // Release the previous round before starting a new one. A regeneration pass
    // used to append to the list without disposing what was already in it,
    // leaking up to _thumbnailCount rasterized ui.Images per pass, and opened a
    // second decoder against the same file without retiring the first.
    for (final stale in _thumbnails) {
      stale?.dispose();
    }
    _thumbnails.clear();
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        ...
        // Must stay mounted while the capture loop runs: the boundary is what
        // _generateThumbnails rasterizes, so gating it on !_generating (which
        // is true for the whole loop) meant every frame read back as null.
        if (_thumbController != null && _thumbController!.value.isInitialized)
          Positioned(
            left: -9999,
            top: -9999,
