...
class ChannelScanService {
    ...
    while (true) {
      ...
      for (final msg in messagesList) {
        ...
        if (contentType == 'messageDocument') {
          ...
          // Skip the app's own metadata documents (manifest + partition JSON).
          // They're uploaded caption-less, so the fallback path below would
          // otherwise turn them into junk timeline items.
  ...
  MediaItem? _buildMediaItem({
    ...
    return MediaItem(
      ...
      // Carry the trash flag from the caption so a scan-based rehydrate does
      // not resurrect a trashed item as un-trashed. (Tombstones — permanent
      // deletions — live in Layer 3 and are honored by the restore engine, not
      // here: captions carry no isDeleted flag.)
