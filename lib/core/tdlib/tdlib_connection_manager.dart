...
class TdLibConnectionManager {
  ...
  Future<void> reconnect() async {
    ...
    // Deliberately NOT resetting _retryCount here: _scheduleReconnect
    // computes the backoff from _retryCount before the timer fires, so a
    // reset inside reconnect would pin every attempt to the 1s initial
    // backoff. Backoff only resets on a real successful connection
    // (connectionStateReady in _listenForUpdates).
      ...
      (update) {
        ...
        if (type == 'updateConnectionState') {
          ...
          if (stateType == 'connectionStateReady') {
            _retryCount = 0;
            // Cancel an armed reconnect timer from a prior outage: the
            // connection is already live, so a late fire would call
            // reconnect() (close + initialize) and churn a working session.
            _stopReconnect();
            _updateStatus(ConnectionStatus.connected);
