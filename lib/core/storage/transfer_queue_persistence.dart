...
class TransferQueuePersistence {
  ...
  /// Exposed for the field-level round-trip test: a task that survives
  /// encode → decode must come back with EVERY persisted field intact.
  /// Serializing through the real file also needs path_provider, which this
  /// avoids.
  @visibleForTesting
  Map<String, dynamic> encodeTask(UploadTask task) => _taskToJson(task);

  /// Inverse of [encodeTask].
  @visibleForTesting
  UploadTask decodeTask(Map<String, dynamic> json) => _taskFromJson(json);
  ...
  Map<String, dynamic> _taskToJson(UploadTask task) {
    return {
      ...
      // Video duration feeds the backup caption. Without it a task that
      // survived a restart wrote a duration-less caption, and the restored
      // video reported 0:00 in the timeline — the exact regression
      // UploadTask.durationMs exists to prevent. Unlike thumbnailPath (an
      // explicitly transient field), this one must round-trip.
