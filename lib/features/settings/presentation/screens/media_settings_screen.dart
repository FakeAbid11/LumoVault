      ...
      body: ListView(
        children: [
          ...
          ListTile(
            ...
            // 0 means "Never delete" (offered further down this screen), so
            // printing it raw read "Trash Duration: 0 days" while the Trash
            // itself said items stay until you delete them — two screens
            // contradicting each other about whether photos get destroyed.
            subtitle: Text(
              settings.trashDurationDays <= 0
                  ? 'Never — keep items until you delete them'
                  : '${settings.trashDurationDays} days',
            ),
...
class _FolderTiles extends StatelessWidget {
  ...
  // Set containment, not a length comparison — see FolderSelectionWidget for
  // why the count version let a stale/synced folder path make "All Folders"
  // read ON while the real selection was something else entirely.
