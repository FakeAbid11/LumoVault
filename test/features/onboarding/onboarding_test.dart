...
void main() {
  group('OnboardingProvider', () {
    ...
    test('progress calculates correctly', () {
      final notifier = OnboardingNotifier();
      expect(notifier.state.progress, closeTo(0.25, 0.01));
      notifier.nextStep();
      expect(notifier.state.progress, closeTo(0.5, 0.01));
    });
  ...
  group('Onboarding Screens', () {
    ...
    testWidgets('folder selection screen shows folders', (tester) async {
      ...
      await tester.pumpWidget(
        ProviderScope(
          overrides: [
            deviceFoldersProvider.overrideWith((_) async => mockFolders),
          ],
          child: const MaterialApp(home: FolderSelectionScreen()),
        ),
      );
      await tester.pumpAndSettle();
