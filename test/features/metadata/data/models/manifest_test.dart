...
void main() {
  group('Manifest', () {
    ...
    test('toJsonString produces valid JSON', () {
      ...
      expect(json, contains('"schema_version":2'));
    ...
    test('isCompatibleWith returns true for equal or newer reader version', () {
      final manifest = Manifest.create(deviceHash: 'hash'); // schemaVersion 2
      expect(manifest.isCompatibleWith(2), isTrue);
      expect(manifest.isCompatibleWith(3), isTrue);
      expect(manifest.isCompatibleWith(1), isFalse);

      // A prior-version (pre-tombstone) manifest still reconciles under the
      // current reader — old channels load as "no tombstones".
      final legacy = Manifest(
        schemaVersion: 1,
        created: DateTime.now().toUtc(),
        deviceHash: 'hash',
        lastSync: DateTime.now().toUtc(),
      );
      expect(legacy.isCompatibleWith(Manifest.currentSchemaVersion), isTrue);
    });
