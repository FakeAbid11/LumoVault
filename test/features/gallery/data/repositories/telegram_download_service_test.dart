...
void main() {
  ...
  group('TelegramDownloadService dispose', () {
    test(
      'fails in-flight downloads, closes the stream, tolerates late updates',
      () async {
        final td = _FakeTd(_documentMessage(fileId: 42));
        final service = _service(td);
        ...
        // A TDLib update landing in the teardown window: the updates
        // subscription may still be live, so the listener runs and hits the
        // now-closed progress controller. Without the isClosed guard this
        // throws a StateError.
        expect(
          () => td.emitPartial(fileId: 42, downloaded: 1, expected: 100),
          returnsNormally,
        );

        // The in-flight future resolves to a cancellation error rather than
        // hanging on a completer nobody will ever complete.
        await expectLater(
          download,
          throwsA(
            isA<TransferError>().having(
        ...
        // The broadcast stream terminated on dispose.
