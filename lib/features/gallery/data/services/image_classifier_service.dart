...
class ImageClassifierService implements AiLabeler {
  ...
  /// callers share one in-flight init instead of each creating a session (the
  /// loser of that race leaked native memory).
  @override
  Future<void> init() {
  ...
  Future<void> dispose() async {
    // _session is `late` and only assigned on a successful init — guard so
    // dispose after a failed init doesn't throw LateInitializationError.
    if (_initialized) {
...
/// Decodes [bytes], resizes to a square [inputSize], and normalizes into the
/// CHW [Float32List] MobileOne-S2 consumes.
///
...
/// classification, and running it on the main isolate dropped frames during a
/// full-library scan. Returns null for undecodable bytes, matching the
/// in-isolate version it replaced.
@pragma('vm:entry-point')
Float32List? decodeAndPreprocessForClassification((Uint8List, int) args) {
  ...
  // channels RGB, stored CHW (channel-first).
