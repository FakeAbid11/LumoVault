...
void main() {
  ...
  group('BackupEngine upload path', () {
    ...
    test(
      'a task cancelled mid-upload is not resurrected by its retry stamp',
      () async {
        // Regression: the deferred-retry path wrote the backoff with
        // `updateTask(getTaskById(id) ?? task)`. updateTask deliberately
        // inserts unknown ids (that is how persisted queues merge), so an
        // upload the user had just cancelled — and which then failed — was
        // re-added to the queue whole-clothed and quietly retried itself.
        final repo = _repo([_item('a'), _item('b')]);
        ...
        final uploads = _FakeUploadService()
          ...
          ..onUpload = (task) {
            if (task.mediaItemId == 'a') {
              cancelledTaskId = task.id;
              // Only the synchronous prefix of cancelTask runs before the
              // failure is handled below — which is exactly the window that
              // used to matter, since removeTask is what it does first.
              engine.cancelTask(task.id);
            }
          };
        engine = _engine(repo: repo, uploads: uploads);
        addTearDown(engine.dispose);
        ...
        expect(cancelledTaskId, isNotNull);
        expect(engine.queue.getTaskById(cancelledTaskId!), isNull);
        expect(
          engine.queue.queuedTasks.where((t) => t.mediaItemId == 'a'),
          isEmpty,
          reason: 'a cancelled task was re-added by its own retry stamp',
        );
        // The run genuinely proceeded, so this is not a vacuous pass.
        expect(uploads.uploadedFileNames, contains('b.jpg'));
      },
    );

    test('queuedPendingCount stays live while stats is throttled', () async {
      // The background runner decided "nothing queued, finish early" from
      // stats.pendingCount, but stats lags the queue by the throttle window —
      // so a run that had just enqueued hundreds of items saw 0 and exited
      // without uploading a single one.
      final repo = _repo([_item('a'), _item('b')]);
      final engine = BackupEngine(
        galleryRepository: repo,
        uploadService: _FakeUploadService(),
        settings: const BackupSettings(uploadDelayMs: 0),
        storageChannelService: StorageChannelService(
          client: TdLibClient.instance,
        )..setCachedChannelId(-100123),
        // A real throttle window, unlike _engine()'s Duration.zero.
        statsThrottleDuration: const Duration(seconds: 30),
      );
      addTearDown(engine.dispose);
      ...
      expect(
        engine.stats.pendingCount,
        0,
        reason: 'stats should still be lagging',
      );
      expect(engine.queuedPendingCount, 2);
    });
