  ...
  group('TelegramAuthRepository', () {
    ...
    group('verifyCode', () {
      test('sends code to TDLib', () async {
        ...
        // TDLib answers checkAuthenticationCode with Ok and only THEN emits the
        // next state, so the result has to come from the settled state — Ready
        // here — not from the Ok, which carries no auth information.
        final resultFuture = repository.verifyCode('12345');
        mockClient.simulateUpdate(const {
      ...
      test('returns password required when the account has 2FA', () async {
        await repository.initialize();
        await repository.sendCode('+1234567890');

        // The Ok for the code means "accepted", but a 2FA account then settles
        // on WaitPassword. Returning AuthSuccess on the Ok navigated into the
        // app while TDLib was still parked in WaitPassword and the connect
        // screen — the only listener for the password state — was disposed.
        final resultFuture = repository.verifyCode('12345');
        mockClient.simulateUpdate(const {
          '@type': 'updateAuthorizationState',
      ...
      test('reports an unregistered number instead of navigating on', () async {
        await repository.initialize();
        await repository.sendCode('+1234567890');
        ...
        expect(result, isA<AuthError>());
        expect((result as AuthError).code, 'REGISTRATION_REQUIRED');
      });
      ...
      test('returns error on invalid code', () async {
        ...
        // The state subscription armed before the throw never completed; drain
        // it so closing the mock's stream in tearDown doesn't surface a
        // spurious "No element" in the next test.
        mockClient.simulateUpdate(const {
          '@type': 'updateAuthorizationState',
    ...
    group('submitPassword', () {
      test('sends password to TDLib', () async {
        ...
        // Same settled-state contract as verifyCode: the Ok precedes the real
        // outcome, so success is only authoritative once Ready arrives.
        final resultFuture = repository.submitPassword('mypassword');
        mockClient.simulateUpdate(const {
          '@type': 'updateAuthorizationState',
          'authorization_state': {'@type': 'authorizationStateReady'},
        });
      ...
      test('returns error on wrong password', () async {
        ...
        // Drain the state subscription armed before the throw — see verifyCode.
        mockClient.simulateUpdate(const {
          '@type': 'updateAuthorizationState',
          'authorization_state': {'@type': 'authorizationStateWaitPassword'},
        });
