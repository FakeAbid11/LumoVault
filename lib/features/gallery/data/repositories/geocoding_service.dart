...
class GeocodingService {
  ...
  final GeocodingHttpClient _httpClient;
  ...
  /// Serializes rate-limited requests. Without chaining, concurrent callers
  /// all read the same [_lastRequest] and fire simultaneously inside one
  /// second, violating Nominatim's 1 req/s policy.
  Future<void>? _rateLimitChain;
  ...
  /// Serialize callers through a 1-second-per-request gap. Each caller waits
  /// for the previous one to clear before computing its own delay, so N
  /// concurrent lookups fire N requests spread over N seconds instead of all
  /// inside one.
  Future<void> _waitForRateLimitSlot() async {
    ...
    try {
      ...
      completer.complete();
      if (identical(_rateLimitChain, completer.future)) {
        _rateLimitChain = null;
      }
    }
  }
  ...
  Future<GeoResult?> reverseGeocode(double lat, double lng) async {
    ...
    // 3. Rate-limit: 1 request per second, serialized across concurrent calls.
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        // A 200 carrying no address block is a genuinely empty spot — cache
        // null so a map pan over open water doesn't re-query every time.
      ...
      // Non-200 (429 rate limit, 5xx, redirect): NOT cached. Any non-200 used
      // to be negative-cached forever, so a single transient server error
      // blanked the spot permanently and clearCache was the only recovery.
      return null;
    } catch (_) {
      // Network/parse failure — same treatment as a non-200: report unknown
      // without negative-caching, so a transient outage retries on the next
      // lookup instead of staying blank.
      return null;
    }
  }
