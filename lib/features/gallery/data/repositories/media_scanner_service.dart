...
class PhotoManagerScannerService implements MediaScannerService {
  ...
  Future<ScanResult> scanDevice({
    ...
    for (final album in albums) {
      ...
      const pageSize = AppConstants.photoManagerPageSize;
  ...
  Future<Uint8List?> getThumbnail(String assetId) async {
    // Ask the platform for this one asset directly. The previous version
    // walked every album's *first page only* (page: 0, size: 100) looking for
    // a match, so any asset past the 100th in its album was simply reported
    // as missing — and the ones it did find cost a full album enumeration.
