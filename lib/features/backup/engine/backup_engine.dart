...
class BackupEngine {
  ...
  Future<void> scanAndEnqueue() async {
    ...
    try {
      ...
      // Immediate, not throttled: callers read `engine.stats` the moment this
      // method returns (BackgroundBackupService._runBackup and
      // _handleMediaScanner both gate on pendingCount) and the 500 ms throttle
      // would hand them the pre-scan snapshot, so a scanner task that just
      // queued work looked empty and the upload never ran.
      ...
      while (!_isPaused) {
        ...
        for (final task in batch) {
          ...
          if (!schedulerResult.canProceed) {
            ...
            // Set the flag, not just the state: isPaused, startBackup()'s
            // re-entry guard, the `while (!_isPaused)` loop below, and
            // backupItemNow's stale-read all key off _isPaused. State alone
            // reported "paused" while isPaused stayed false, so an auto-resume
            // or a resume() call re-entered startBackup without the user-pause
            // semantics and double-drove the queue.
            _isPaused = true;
            _setState(BackupEngineState.paused);
            _scheduleAutoResume();
            _updateStats();
  ...
  Future<void> _uploadTask(UploadTask task) async {
    ...
    try {
      ...
      // Retry budget: a task gets at most UploadTask.maxAttempts transfer
      // attempts. retryCount is pre-increment (the +1 below counts the
      // attempt that just failed), so a plain `retryCount < maxAttempts`
      // allowed one extra transfer — maxAttempts=3 actually ran 4 uploads
      // (retryCount 0,1,2,3 all passing the guard). This is independent of
      // uploadBatchSize, which is a throughput knob, not a retry limit.
      final shouldRetry =
          e.retryable && task.retryCount + 1 < UploadTask.maxAttempts;
      ...
      if (shouldRetry) {
        ...
        // Route through _updateTaskIfQueued, not _queue.updateTask: the raw
        // `?? task` fallback re-inserts the stale snapshot when the task was
        // cancelled in the window since the failure handler began, and
        // UploadQueue.updateTask inserts unknowns by design — resurrecting a
        // task the user (or cancelTask) already removed.
        _updateTaskIfQueued(
          task.id,
          (current) => current.copyWith(nextAttemptAt: wakeAt),
        );
        debugPrint(
  ...
  void _persistQueue() {
    final persistence = queuePersistence;
    if (persistence == null) return;
    // Snapshot: [allTasks] is an unmodifiable *view* of the backing list, not
    // a copy, so a dispose() that clears the queue while the async save is
    // still in flight would persist an empty list.
    final tasks = List<UploadTask>.of(_queue.allTasks);
    unawaited(
      persistence.saveTasks(tasks).catchError((Object e) {
        debugPrint('[BackupEngine] Failed to persist queue: $e');
      }),
    );
  }

  /// Flush an armed persist immediately instead of waiting on the 5 s
  /// [_persistTimer].
  ///
  /// [dispose] calls this so a queue populated moments earlier isn't lost:
  /// teardown cancels [_persistTimer] before its callback ever runs, and with
  /// it the only in-memory record of those task ids.
  void flushQueue() {
    if (_disposed) return;
  ...
  void dispose() {
    if (_disposed) return;
    // Flush BEFORE marking disposed (flushQueue no-ops once _disposed is set):
    // the scanner task can enqueue and then have the container tear the engine
    // down within the 5 s persist window, which would otherwise cancel the
    // only armed save and drop the queue.
    flushQueue();
    _disposed = true;
    _isProcessingQueue = false;
    _retryTimer?.cancel();
