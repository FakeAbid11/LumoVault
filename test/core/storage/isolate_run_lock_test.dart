...
void main() {
  ...
  test('a heartbeat rewrite is never observed as an abandoned lock', () async {
    ...
    for (var i = 1; i <= 20; i++) {
      ...
      expect(
        // File format is "<owner token> <iso-8601>"; the point of the assertion
        // is that a concurrent reader never sees the empty/partial intermediate.
        DateTime.tryParse(seen.trim().split(' ').last),
        isNotNull,
  ...
  test(
    'a takeover survivor does not free the new holder when it releases',
    () async {
      // The original holder's lock went stale, a second isolate took over, and
      // the first one finishing used to delete the file unconditionally —
      // freeing a run that was still going and admitting a third isolate
      // alongside it for concurrent TDLib access.
      final abandoned = makeLock(staleAfter: const Duration(minutes: 10));
      final takeover = makeLock(staleAfter: const Duration(minutes: 10));
      final bystander = makeLock(staleAfter: const Duration(minutes: 10));
      final start = DateTime(2026, 3, 1, 12);

      await abandoned.tryAcquire(now: start);
      final tookOverAt = start.add(const Duration(minutes: 11));
      expect(await takeover.tryAcquire(now: tookOverAt), isTrue);

      // The abandoned-but-still-running original finishes late and releases.
      await abandoned.release();

      expect(
        await bystander.tryAcquire(now: tookOverAt),
        isFalse,
        reason: 'release() deleted a lock it no longer owned',
      );
    },
  );

  test('a beaten-over holder stops writing to the lock file', () async {
    final abandoned = makeLock(staleAfter: const Duration(minutes: 10));
    final takeover = makeLock(staleAfter: const Duration(minutes: 10));
    final third = makeLock(staleAfter: const Duration(minutes: 10));
    final start = DateTime(2026, 3, 1, 12);
    ...
    // A late heartbeat from the displaced holder must not steal back or
    // extend a claim it no longer owns.
    await abandoned.heartbeat(now: start.add(const Duration(minutes: 30)));
    expect(abandoned.isHeld, isFalse);

    // Proof the file still carries the takeover's timestamp (11 min), not the
    // bogus 30-min one: at 25 min the takeover's lock is stale and a new
    // holder can claim it.
    expect(
      await third.tryAcquire(now: start.add(const Duration(minutes: 25))),
      isTrue,
    );
  });
