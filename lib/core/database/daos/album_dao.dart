...
class AlbumDao extends DatabaseAccessor<AppDatabase> with _$AlbumDaoMixin {
  ...
  /// Delete an album and its membership rows.
  ///
  /// Wrapped because `album_items` has no enforced foreign key: two separate
  /// statements meant a crash in between left orphan membership rows pointing
  /// at an album that no longer exists, which inflate album counts forever.
  Future<void> deleteAlbum(int albumId) => transaction(() async {
