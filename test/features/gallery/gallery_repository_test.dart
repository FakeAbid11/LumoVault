...
void main() {
  ...
  group('GalleryRepository', () {
    ...
    test('getTimelineItems excludes hidden and trashed by default', () async {
      final items = [
        createTestMediaItem(localId: '1'),
        createTestMediaItem(localId: '2'),
        createTestMediaItem(localId: '3', isHidden: true),
        createTestMediaItem(localId: '4', isTrashed: true),
      ];
      ...
      expect(timeline.map((i) => i.localId).toSet(), {'1', '2'});
    });

    test('getTimelineItems returns hidden items when requested', () async {
      final items = [
        createTestMediaItem(localId: '1'),
        createTestMediaItem(localId: '2', isHidden: true),
        createTestMediaItem(localId: '3', isTrashed: true),
      ];

      mockScanner.setMediaItems(items);
      await repository.scanDevice();

      final hidden = repository.getTimelineItems(isHidden: true);
      expect(hidden.map((i) => i.localId).toSet(), {'2'});
      final trashed = repository.getTimelineItems(isTrashed: true);
      expect(trashed.map((i) => i.localId).toSet(), {'3'});
    });
    ...
    test('getAlbumItems excludes hidden and trashed items', () async {
      final items = [
        createTestMediaItem(localId: '1', albumName: 'Camera'),
        createTestMediaItem(localId: '2', albumName: 'Camera', isHidden: true),
        createTestMediaItem(localId: '3', albumName: 'Camera', isTrashed: true),
      ];
      ...
      final cameraItems = repository.getAlbumItems('Camera');
      expect(cameraItems.map((i) => i.localId).toSet(), {'1'});
    });

    test('getFavoriteItems excludes hidden and trashed items', () async {
      final items = [
        createTestMediaItem(localId: '1', isFavorite: true),
        createTestMediaItem(localId: '2', isFavorite: true, isHidden: true),
        createTestMediaItem(localId: '3', isFavorite: true, isTrashed: true),
        createTestMediaItem(localId: '4'),
      ];

      mockScanner.setMediaItems(items);
      await repository.scanDevice();

      final favorites = repository.getFavoriteItems();
      expect(favorites.map((i) => i.localId).toSet(), {'1'});
    });
    ...
    test('searchMedia excludes hidden and trashed items', () async {
      final items = [
        createTestMediaItem(localId: '1', fileName: 'vacation.jpg'),
        createTestMediaItem(
          localId: '2',
      ...
      final results = repository.searchMedia('vacation');
      expect(results.map((i) => i.localId).toSet(), {'1'});
    });
    ...
    test('getTimelineByDate excludes hidden and trashed items', () async {
      final now = DateTime.now();
      final today = DateTime(now.year, now.month, now.day);
      ...
      final grouped = repository.getTimelineByDate();
      expect(grouped['Today']?.map((i) => i.localId).toSet(), {'1'});
    });
    ...
    test('metadata edits bump modifiedAt for the sync layer', () async {
      ...
      // The partition content hash and LWW conflict resolution both key off
      // modifiedAt, so an edit that leaves it alone never reaches the channel.
      await repository.setCreatedAt('1', DateTime(2025, 6, 1));
      await repository.setTags('2', const ['vacation']);
      await repository.labelAnyMediaItem('3', const ['ai_beach']);

      expect(repository.getItemById('1')!.modifiedAt, isNot(old));
      expect(repository.getItemById('2')!.modifiedAt, isNot(old));
      expect(repository.getItemById('3')!.modifiedAt, isNot(old));
      expect(repository.getItemById('1')!.isDateUserSet, isTrue);
    });
  });
}
