...
void main() {
  group('ClipTokenizer (mini vocab)', () {
    ...
    test('non-ASCII tokens byte-encode instead of vanishing', () {
      // 'я' (U+044F) is two UTF-8 bytes, each a vocab byte char. Before the
      // byte-encode step the raw token was absent from the vocab and the null
      // check dropped it, so the query encoded to SOT EOT padding — semantic
      // search returned nothing for any Cyrillic/CJK/emoji query.
      final tokens = _tokenizer().tokenize('я');
      expect(tokens.first, 49406); // SOT
      expect(tokens[1], isNot(0)); // first byte char
      expect(tokens[2], isNot(0)); // second byte char
      expect(tokens[3], 49407); // EOT directly after — nothing was dropped
    });

    test('a 4-byte emoji byte-encodes to four ids', () {
      // '😀' (U+1F600) = 4 UTF-8 bytes -> 4 byte chars, none of which the mini
      // merges combine, so EOT lands right after them.
      final tokens = _tokenizer().tokenize('😀');
      expect(tokens.first, 49406);
      for (var i = 1; i <= 4; i++) {
        expect(tokens[i], isNot(0));
      }
      expect(tokens[5], 49407);
    });
  });
}
