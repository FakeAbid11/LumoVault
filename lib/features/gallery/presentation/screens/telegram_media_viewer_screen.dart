...
class _TelegramMediaViewerScreenState
              ...
              : AppBar(
                  ...
                  actions: [
                    ...
                    IconButton(
                      ...
                      tooltip: 'Details',
                      onPressed: () => _showDetails(currentItem),
                    ),
                  ],
                ),
          body: SwipeDismissWrapper(
            enabled: !_isZoomed,
            onSwipeUp: () => _showDetails(currentItem),
  ...
  /// Backup detail sheet, which is the honest primary info view for a
  /// cloud-only item: EXIF needs the original file, which may not be
  /// downloaded yet. Camera details stay one tap away inside it.
  void _showDetails(MediaItem item) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (sheetContext) => TelegramItemDetailSheet(
        item: item,
        onShowExif: () {
          Navigator.of(sheetContext).pop();
          _showExifDetails(item);
        },
      ),
    );
  }
...
///
/// Opened by [TelegramMediaViewerScreen]'s info action and its swipe-up
/// gesture. [onShowExif] hands off to the camera-details sheet — omit it where
/// the original file isn't reachable and the button is left out.
class TelegramItemDetailSheet extends StatelessWidget {
  const TelegramItemDetailSheet({
    super.key,
    required this.item,
    this.onShowExif,
  });
  ...
  /// Opens the EXIF sheet for the same item, when the caller can offer it.
            ...
            if (onShowExif != null) ...[
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  onPressed: onShowExif,
                  icon: const Icon(Symbols.photo_camera),
                  label: const Text('Camera & EXIF details'),
                ),
              ),
