...
class MetadataRepository {
  ...
  /// Upgrades a manifest pulled from Telegram to the current schema before it
  /// is applied as the sync baseline. Defaults to a plain [MigrationService]
  /// for callers that construct the repository directly.
  ...
  Future<void> initialize() async {
    // Restore the partition set and the manifest before reading layer 1 or
    // the sync log: `_loadLocalMetadata` iterates the partition map, and the
    // manifest owns the sync baseline (`_partitionHashes`). Loading neither
    // (as an earlier version did) left both empty on every cold start —
    // layer 1 forgot every edit until a rescan, the pull side treated every
    // remote chunk as changed, and any push ahead of the pull re-uploaded
    // the full metadata set. Each launch was a full sync instead of an
    // incremental one.
  ...
  Future<void> recordStateChange({
    ...
    if (updatedItem != null) {
      // A state change can arrive carrying a stale [MediaItem] — e.g. the
      // viewer's snapshot from before an upload finished. PartitionItem's
      // telegram pointers are hash-excluded, so overwriting them with nulls
      // would silently orphan the channel reference and no later change would
      // repair it. Preserve whatever the live record already resolved.
      ...
      _localMetadata[localId] = effective;
      partitionService.upsertItem(effective);
      searchIndexService.reindexItem(effective);
    }
  ...
  Future<void> recordUploadComplete({
    ...
    // An upload that lands after a permanent delete would write fresh
    // pointers onto the tombstone, re-dirtying the partition and pushing the
    // resurrection to every other device. The delete wins; the orphaned
    // message is left on the channel (a later delete-forever revokes it).
    if (existing.isDeleted) {
      debugPrint(
        '[MetadataRepository] Skipping upload-complete for tombstoned '
        '$localId',
      );
      return;
    }
  ...
  Future<int> reconcileFromTelegram({
    ...
    final downloaded = await downloadManifest();
    if (downloaded == null) return 0;

    // Upgrade the remote manifest to the current schema before trusting it as
    // the sync baseline. A null result means the format is only partly
    // migratable — refuse rather than apply a manifest we cannot fully
    // interpret, and leave the local baseline untouched.
    final remoteManifest = migrationService.migrateManifest(downloaded);
    if (remoteManifest == null) {
      debugPrint(
        '[MetadataRepository] Refused remote manifest at schema version '
        '${downloaded.schemaVersion}; keeping local baseline',
      );
      return 0;
    }
...
class MetadataSyncStatus {
  ...
  MetadataSyncStatus copyWith({
    ...
    return MetadataSyncStatus(
      ...
      // Unlike the other fields, an error is *cleared* by passing null — so a
      // plain `?? this.syncError` made every copyWith that didn't mention the
      // error wipe the recorded failure from the UI. Take an explicit opt-in
      // to clear it instead.
      syncError: clearSyncError
          ? null
          : (syncError is String ? syncError : this.syncError),
