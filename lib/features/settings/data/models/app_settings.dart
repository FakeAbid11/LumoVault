...
class AppSettings {
  ...
  factory AppSettings.fromJsonString(String jsonString) {
    try {
      ...
      return AppSettings(
        ...
        gridSize: _parseGridSize(map['gridSize']),
  ...
  /// Parse [ThemeMode] from a string name (preferred) or int index
  ...
  /// Parse [GridSize] from a string name (preferred) or int index
  /// (backward compatibility).
  static GridSize _parseGridSize(Object? value) {
    if (value is String) {
      return GridSize.values.firstWhere(
        (g) => g.name == value,
        orElse: () => GridSize.medium,
      );
    }
    if (value is int) {
      return (value >= 0 && value < GridSize.values.length)
          ? GridSize.values[value]
          : GridSize.medium;
    }
  ...
  bool operator ==(Object other) =>
      ...
      other is AppSettings &&
          ...
          minBatteryLevel == other.minBatteryLevel &&
          backgroundBackupEnabled == other.backgroundBackupEnabled &&
          maxParallelUploads == other.maxParallelUploads &&
          backupVideos == other.backupVideos &&
          backupPhotos == other.backupPhotos &&
          const DeepCollectionEquality().equals(
            includedFolders,
            other.includedFolders,
          ) &&
          const DeepCollectionEquality().equals(
            excludedFolders,
            other.excludedFolders,
          ) &&
          const DeepCollectionEquality().equals(
            excludedFileHashes,
            other.excludedFileHashes,
          ) &&
          uploadBatchSize == other.uploadBatchSize &&
          uploadDelayMs == other.uploadDelayMs &&
          maxFileSizeBytes == other.maxFileSizeBytes &&
          storageChannelId == other.storageChannelId &&
          lastBackupAt == other.lastBackupAt &&
          lastScanAt == other.lastScanAt &&
          trashDurationDays == other.trashDurationDays &&
          ...
          requireAuthOnAppOpen == other.requireAuthOnAppOpen &&
  ...
  int get hashCode => Object.hashAll([
    ...
    backupVideos,
    backupPhotos,
    Object.hashAll(includedFolders),
    Object.hashAll(excludedFolders),
    Object.hashAll(excludedFileHashes),
    uploadBatchSize,
    uploadDelayMs,
    maxFileSizeBytes,
    storageChannelId,
