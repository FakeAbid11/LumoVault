...
class FaceRepository {
      ...
      for (var i = 0; i < detectedFaces.length; i++) {
        ...
        companions.add(
          FacesCompanion.insert(
            ...
            embeddingModel: Value(faceDetectionService.embedderModelTag),
  ...
  Future<int> clusterFaces() async {
    ...
    // A cohort bigger than [maxFacesPerClusterPass] is only sliced once per
    // pass, so the tail beyond the first slice was never clustered unless the
    // front happened to mint people that then absorbed it via
    // reclusterOrphans — and a front slice of all-sub-minimum clusters
    // stranded the entire tail permanently. Repeat the whole cycle, re-querying
    // the unassigned set each pass so faces assigned in an earlier pass are
    // excluded, until the backlog fits one slice or stops making progress.
    const maxPasses = 8;
    var totalNewPersons = 0;
    for (var pass = 0; pass < maxPasses; pass++) {
      final unassigned = await faceDao.unassignedFaces();
      if (unassigned.isEmpty) break;
      final allWithEmb = unassigned
          .where((f) => f.embedding.isNotEmpty)
          .toList();
      if (allWithEmb.isEmpty) break;

      final newPersons = await _clusterCohorts(allWithEmb);
      totalNewPersons += newPersons;

      // The slice just ran against everything that was left: backlog drained.
      if (allWithEmb.length <= maxFacesPerClusterPass) break;
      // The front slice minted nobody — it is all sub-minimum clusters, so the
      // next pass would re-slice the same faces and never reach the tail.
      if (newPersons == 0) break;
    }
    ...
    if (totalNewPersons > 0) {
      await reclusterOrphans();
    }
    onAssignmentsChanged?.call();
    return totalNewPersons;
  }

  /// Split the unassigned backlog by embedder model and cluster each cohort.
  ///
  /// v20: cohort by embedder model before clustering — average-linkage and the
  /// refinement sweep must never mix vectors from different spaces, and a
  /// person's centroid must be tagged with the space it lives in.
  Future<int> _clusterCohorts(List<FaceRow> allWithEmb) async {
    final byModel = <String, List<FaceRow>>{};
    for (final face in allWithEmb) {
      byModel.putIfAbsent(face.embeddingModel, () => []).add(face);
    }
    var newPersons = 0;
    for (final cohort in byModel.values) {
      newPersons += await _clusterFacesCohort(cohort);
    }
    return newPersons;
  }

  /// Clusters one same-model cohort (oldest-first slice of
  /// [maxFacesPerClusterPass]) and mints people from clusters big enough to
  /// stand alone. Returns the number of people created.
  Future<int> _clusterFacesCohort(List<FaceRow> allWithEmb) async {
    ...
    for (final cluster in clusters) {
      ...
      await faceDao.updateCentroid(
        ...
        model: withEmb[cluster.first].embeddingModel,
      );
  ...
  Future<void> reclusterOrphans() async {
    ...
    // Member faces are prefetched once per anchor for the whole pass — they
    // are only consulted to CONFIRM a centroid match (the per-member cosine
    // runs only after the cheap centroid gate passes).
    final membersByPerson = <int, List<FaceRow>>{
      for (final person in anchors)
        person.id: await faceDao.facesForPerson(person.id),
    };
    for (final orphan in orphanEmb) {
      var bestId = -1;
      var bestSim = 0.0;
      for (final person in anchors) {
        // v20 provenance gate: two ONNX embedders produce vectors in
        // different spaces — cross-model similarity is meaningless, so a
        // face may only ever join a person tagged with the SAME embedder.
        if (person.centroidModel != orphan.embeddingModel) continue;
        ...
        final threshold = absorbThreshold(person.name);
        if (sim < threshold) continue;

        // Centroid similarity is systematically INFLATED relative to
        // face-to-face similarity (averaging cancels per-face noise), which
        // is how a single mediocre face permanently joined a stranger's
        // person at 0.45. Require at least one real member face to clear the
        // same bar. An anchor with no faces yet (e.g. a freshly-named person
        // after a rescan keeps its centroid but lost its members) is allowed.
        final members = membersByPerson[person.id] ?? const <FaceRow>[];
        final confirmed = members
            .where((m) => m.embedding.isNotEmpty)
            .any(
              (m) =>
                  faceClusteringService.cosineSimilarity(
                    orphan.embedding,
                    m.embedding,
                  ) >=
                  threshold,
            );
        if (members.isNotEmpty && !confirmed) continue;
      ...
      if (bestId >= 0) {
        ...
        // The absorbing person's centroid now describes one more member, but
        // the loop above matched this orphan against the PRE-assignment
        // centroid. Leave it stale and every later orphan in this same pass
        // is compared against a centroid that no longer represents the
        // cluster. Every other membership change in this repository already
        // recomputes; this was the one that didn't.
        await recomputeCentroid(bestId);
      }
    }
    onAssignmentsChanged?.call();
  }
  ...
  Future<int> consolidateDuplicatePeople() async {
    ...
    for (var i = 0; i < candidates.length; i++) {
      ...
      for (var j = i + 1; j < candidates.length; j++) {
        ...
        // Cross-model centroids are incomparable (v20): never merge across
        // embedders regardless of score.
        if (target.centroidModel != source.centroidModel) continue;
        ...
        // Centroid-vs-centroid is the MOST inflated comparison in the whole
        // pipeline (both operands are noise-cancelled means), so a passing
        // centroid score alone used to merge two genuinely different people.
        // Require at least one real face-to-face pair above the same bar to
        // confirm the two clusters share an actual identity. A cluster whose
        // only claim is averaged proximity stays separate — the whole point.
        if (!await _shareFacePairAbove(target.id, source.id, threshold)) {
          continue;
        }
  ...
  /// True when at least one face of [aId] and one face of [bId] score
  /// cosine >= [threshold] against EACH OTHER (real face-to-face, not
  /// centroid-vs-centroid). Sampled at 50 faces per side to bound the
  /// pairwise cost — a person with >50 stored faces is far past the point
  /// where sampling decides anything. A side with no usable faces cannot
  /// confirm anything and returns false (the merge is refused).
  Future<bool> _shareFacePairAbove(int aId, int bId, double threshold) async {
    const sampleCap = 50;
    final aFaces = (await faceDao.facesForPerson(
      aId,
    )).where((f) => f.embedding.isNotEmpty).take(sampleCap).toList();
    final bFaces = (await faceDao.facesForPerson(
      bId,
    )).where((f) => f.embedding.isNotEmpty).take(sampleCap).toList();
    if (aFaces.isEmpty || bFaces.isEmpty) return false;
    for (final a in aFaces) {
      for (final b in bFaces) {
        if (faceClusteringService.cosineSimilarity(a.embedding, b.embedding) >=
            threshold) {
          return true;
        }
      }
  ...
  Future<void> recomputeCentroid(int personId) async {
    ...
    // Members are model-homogeneous by construction (absorption refuses
    // cross-model faces since v20); stamp with theirs.
    final model = faces
        .map((f) => f.embeddingModel)
  ...
  /// User correction — "this is not the person". Unassigns [personId]'s
  /// faces on the given photos AND excludes them, so the hardened rules (or
  /// a future rescan pass) never re-merge the same mistake. Faces of OTHER
  /// people in those photos are untouched: a two-person photo correctly
  /// stays in the other person's group.
  ///
  /// Returns the number of faces removed.
  Future<int> removeFacesFromPerson(
    int personId,
    List<String> mediaItemIds,
  ) async {
    final mine = await faceDao.facesForPerson(personId);
    final wanted = mediaItemIds.toSet();
    final doomed = mine
        .where((f) => wanted.contains(f.mediaItemId))
        .map((f) => f.id)
        .whereType<int>()
        .toList();
    if (doomed.isEmpty) return 0;
    await faceDao.excludeFaces(doomed);
    await _settlePerson(personId);
    onAssignmentsChanged?.call();
    return doomed.length;
  }

  /// User correction — "move to another person…". Reassigns [personId]'s
  /// faces on the given photos to [targetPersonId], or to a freshly created
  /// (unnamed) person when null. Moved faces are NOT excluded — they can be
  /// corrected again later.
  ///
  /// Returns the number of faces moved.
  Future<int> moveFacesToPerson(
    int personId,
    List<String> mediaItemIds,
    int? targetPersonId,
  ) async {
    ...
    final target = targetPersonId ?? await faceDao.createPerson(null);
    await faceDao.assignFacesToPerson(moving, target);
    await _settlePerson(target);
    await _settlePerson(personId);
    onAssignmentsChanged?.call();
    return moving.length;
  }

  /// One-time cleanup for groups formed under the OLD loose rules: every
  /// member face that could not pass today's absorption bar against its own
  /// person's centroid (or whose embedder tag no longer matches) is
  /// un-assigned — not excluded — then the hardened absorption re-homes
  /// whatever genuinely belongs somewhere else.
  ///
  /// Returns the number of faces un-assigned.
  Future<int> requalitySweep() async {
    var removed = 0;
    for (final person in await faceDao.allPeopleRows()) {
      if (person.centroidEmbedding.isEmpty) continue;
      final bar = absorbThreshold(person.name);
      final members = await faceDao.facesForPerson(person.id);
      final weak = <int>[];
      for (final f in members) {
        if (f.excluded) continue;
        if (f.embedding.isEmpty ||
            f.embeddingModel != person.centroidModel ||
            faceClusteringService.cosineSimilarity(
                  f.embedding,
                  person.centroidEmbedding,
                ) <
      ...
      await faceDao.unassignFaces(weak);
      removed += weak.length;
      await _settlePerson(person.id);
    }
    if (removed > 0) {
      // The hardened rules get first crack at re-homing the freed faces.
      await reclusterOrphans();
    }
    onAssignmentsChanged?.call();
    return removed;
  ...
  /// Recompute centroid + thumbnail after membership changed, and retire the
  /// anchor properly when nothing is left: an unnamed person with no faces is
  /// deleted outright (its stale centroid could otherwise still absorb at the
  /// centroid-only bar), a named one keeps its name with an empty centroid.
  Future<void> _settlePerson(int personId) async {
    final faces = await faceDao.facesForPerson(personId);
    final members = faces.where((f) => f.embedding.isNotEmpty).toList();
    if (members.isEmpty) {
      final person = await faceDao.personById(personId);
      if (person != null && !_hasName(person.name)) {
        await faceDao.deletePerson(personId);
        return;
      }
      await faceDao.updateCentroid(personId, const []);
      await faceDao.setPersonThumbnail(personId, null);
      return;
    }
    await recomputeCentroid(personId);
    final person = await faceDao.personById(personId);
    final thumbStillMine =
        person?.thumbnailFaceId != null &&
        members.any((f) => f.id == person!.thumbnailFaceId);
    if (!thumbStillMine) {
      final biggest = members.reduce((a, b) {
        final aa = a.boundingBoxWidth * a.boundingBoxHeight;
        final ab = b.boundingBoxWidth * b.boundingBoxHeight;
        return aa >= ab ? a : b;
      });
      await faceDao.setPersonThumbnail(personId, biggest.id);
    }
  }
