...
class _ExifDetailsSheetState extends ConsumerState<ExifDetailsSheet> {
  ...
  /// The date edited from this sheet, if any. The snapshot sources in [build]
  /// (AssetEntity / the immutable MediaItem) don't change after [setCreatedAt],
  /// so without this an edit left the sheet showing the stale date until it
  /// was reopened.
  ...
  Future<void> _loadFileDetails() async {
    if (widget.asset != null) {
      File? fetched;
      try {
        fetched = await widget.asset!.file;
      } catch (_) {
        // The asset's file is gone (deleted mid-load) or the platform lookup
        // fails: degrade to whatever the MediaItem snapshot already carries
        // instead of killing the whole sheet.
        fetched = null;
      }
      // Copy to a non-nullable-promoted final so the closures below see a
      // non-null File (a nullable local loses promotion across them).
      final file = fetched;
      if (file != null && mounted) {
        try {
          ...
          // The file exists but can't be stat'd (permissions, a race with
          // deletion): leave size/path empty and render the rest of the sheet.
        }
  ...
  Future<void> _editDate(BuildContext context, DateTime currentDate) async {
    ...
    if (mounted) {
      // Reflect the edit now: the snapshot sources in build don't change
      // after setCreatedAt, so an empty setState would keep showing the
      // pre-edit date until the sheet was closed and reopened.
      _editedDate = newDate;
      setState(() {});
    }
  }
