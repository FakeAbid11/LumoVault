...
class BackupEngineNotifier extends StateNotifier<BackupEngineState> {
  ...
  /// Live pending-task count — see [BackupEngine.queuedPendingCount] for why
  /// this is not [stats], which is throttled and can read stale.
  int get queuedPendingCount => _engine.queuedPendingCount;
...
final autoScanSyncProvider = Provider<void>((ref) {
  ...
  // One sequential batch rather than three independent fire-and-forget calls.
  // None of them awaited initialize() — which only registerAllTasks performs,
  // and that itself was unawaited from bootstrap — so on a cold start these
  // could race ahead of WorkManager init, throw MissingPluginException as an
  // unhandled async error, and leave the AI / face / CLIP tasks unregistered
  // for the whole session with nothing logged.
  unawaited(() async {
    try {
      await service.initialize();
      await (settings.aiScanEnabled
          ? service.registerAiScan()
          : service.cancelTask(kAiScanTask));
      await (settings.faceScanEnabled
          ? service.registerFaceScan()
          : service.cancelTask(kFaceScanTask));
      // Always registered — this is what makes semantic search work.
      await service.registerClipEmbedding();
    } catch (e) {
      debugPrint('[AutoScanSync] Could not apply scan registrations: $e');
    }
  }());
});
