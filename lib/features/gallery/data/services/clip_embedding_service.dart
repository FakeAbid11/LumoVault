...
///
/// Uses MobileCLIP2-S0 (Apple's latest mobile-optimized CLIP model). The image
/// tower ships at `assets/models/mobileclip2_s0_vision.onnx`; the text tower
/// is at `assets/models/mobileclip2_s0_text_int8.onnx` and lazily loaded on
/// the first text search — when it's absent, text embedding fails gracefully.
class ClipEmbeddingService implements AiTextEmbedder {
  ...
  Future<void> _initNow() async {
    try {
      ...
      _session = await _ort.createSessionFromAsset(
        'assets/models/mobileclip2_s0_vision.onnx',
  ...
  static const String _textModelAsset =
      'assets/models/mobileclip2_s0_text_int8.onnx';
...
/// Decodes [bytes], resizes to a square [inputSize], and scales pixels into
/// the [0, 1] CHW [Float32List] MobileCLIP2-S0 consumes.
///
/// Top-level with a `vm:entry-point` pragma so it survives tree-shaking when
/// handed to [compute]: decode + resize + normalize is the expensive half of
/// embedding, and running it on the main isolate dropped frames during a
/// full-library scan. Returns null for undecodable bytes, matching the
/// in-isolate version it replaced.
@pragma('vm:entry-point')
Float32List? decodeAndPreprocessForEmbedding((Uint8List, int) args) {
  final (bytes, inputSize) = args;
  final decoded = img.decodeImage(bytes);
  if (decoded == null) return null;

  final resized = img.copyResize(decoded, width: inputSize, height: inputSize);

  // MobileCLIP2-S0 expects pixels scaled to [0, 1] (no mean/std shift — the
  // model's own normalization is identity) in CHW (channel-first) layout.
