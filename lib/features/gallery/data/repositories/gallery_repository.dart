...
class GalleryRepository {
  ...
  /// Index at which [item] keeps [_mediaItems] sorted newest-first by
  /// [MediaItem.createdAt] — the first position whose existing date is not
  /// strictly newer. Binary search, so the list must already be sorted; every
  /// insert path goes through here or through a full `_mediaItems.sort`, which
  /// is what maintains that invariant.
  int _insertionIndex(MediaItem item) {
    var lo = 0;
    var hi = _mediaItems.length;
    while (lo < hi) {
      final mid = lo + ((hi - lo) >> 1);
      if (_mediaItems[mid].createdAt.compareTo(item.createdAt) > 0) {
        lo = mid + 1;
      } else {
        hi = mid;
      }
  ...
  /// Insert [item] in capture-date order and refresh the index.
  ///
  /// The timeline is a date-sorted read model. The bulk paths (hydrate,
  /// scanDevice, scanDeviceIncremental, setCreatedAt) sort the whole list
  /// afterwards, but the single-item paths used to plain-append and leave the
  /// list unsorted until the next full sort — so a photo labeled or favorited
  /// mid-library rendered in the wrong position and jumped later.
  void _addSorted(MediaItem item) {
    final index = _insertionIndex(item);
    _mediaItems.insert(index, item);
    _indexByLocalId[item.localId] = index;
  }
  ...
  /// Pass null for both to clear a previously user-set location — coordinates
  /// and the reverse-geocoded name are nulled together, so the pin cannot
  /// linger while the item is demoted to "not user-set" (which would let a
  /// rescan silently overwrite it).
  Future<void> setLocation(
    ...
    if (index != -1) {
      ...
      final updated = _mediaItems[index].copyWith(
        ...
        // Bump modifiedAt so the edit is a real change for LWW conflict
        // resolution AND for the partition content hash: a location edit that
        // leaves modifiedAt alone is invisible to the metadata sync layer.
  ...
  Future<void> setCreatedAt(
    ...
    if (index != -1) {
      final updated = _mediaItems[index].copyWith(
        ...
        // Bump modifiedAt: the partition content hash and LWW conflict
        // resolution both key off it, so a date edit that leaves it alone is
        // invisible to the metadata sync layer.
        modifiedAt: DateTime.now(),
      );
      _mediaItems[index] = updated;
  ...
  Future<void> labelAnyMediaItem(String localId, List<String> labels) async {
    ...
    if (index != -1) {
      final updated = _mediaItems[index].copyWith(
        aiLabels: labels,
        // Bump modifiedAt so the label change reaches the metadata sync
        // layer: the partition content hash and LWW both key off it.
        modifiedAt: DateTime.now(),
      );
      _mediaItems[index] = updated;
      try {
