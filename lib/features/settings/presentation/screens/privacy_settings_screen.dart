          ...
          SwitchListTile(
            ...
            subtitle: Text(
              settings.pinLockEnabled && settings.pinHash != null
                  ...
                  : 'Set a PIN first — biometrics alone have no fallback',
            ),
            value: settings.biometricLockEnabled,
            onChanged: (value) {
              if (value &&
                  !(settings.pinLockEnabled && settings.pinHash != null)) {
                // A biometric-only lock is unresolvable the moment biometrics
                // can't authenticate — a re-enrolled finger, a failing sensor,
                // or a vendor auth service refusing the call — because the lock
                // screen renders its PIN field only when a PIN exists, and the
                // gate covers the whole app. That is a permanent lockout from
                // the user's only copy of their photos, so the combination is
                // not offered.
                showLumoSnackBar(
                  context,
                  'Set a PIN before enabling biometric lock.',
                );
                return;
              }
              ref
