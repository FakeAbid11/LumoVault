...
class MediaItems extends Table {
  ...
  /// Photo GPS coordinates read from EXIF (photo_manager `latlngAsync`).
  /// Nullable: many photos carry no location fix. The Map tab plots only rows
...
class AppDatabase extends _$AppDatabase {
  ...
  MigrationStrategy get migration => MigrationStrategy(
    ...
    onUpgrade: (m, from, to) async {
      ...
      // v4 -> v5: photo GPS coordinates for the Map tab.
      if (from < 5) {
        await m.addColumn(mediaItems, mediaItems.latitude);
        await m.addColumn(mediaItems, mediaItems.longitude);
      }
      // v5 -> v6: user-set location flag so manually-set coordinates survive
      // rescans.
      if (from < 6) {
