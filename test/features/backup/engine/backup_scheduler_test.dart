...
void main() {
  group('BackupScheduler', () {
    ...
    group('filterItemsForBackup', () {
      test('filters out hidden items', () {
        ...
        expect(filtered.map((i) => i.localId).toSet(), {'1'});
      });
  ...
  group('BackupSettings', () {
    ...
    test('settings differing only in uploadBatchSize are not equal', () {
      const a = BackupSettings(uploadBatchSize: 10);
      const b = BackupSettings(uploadBatchSize: 20);
      expect(a, isNot(equals(b)));
    });

    test('settings differing only in excludedFolders are not equal', () {
      const a = BackupSettings(excludedFolders: ['a', 'b']);
      const b = BackupSettings(excludedFolders: ['a', 'c']);
      expect(a, isNot(equals(b)));
    });
  });
}
