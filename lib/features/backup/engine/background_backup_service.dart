...
class BackgroundTaskRunner {
  ...
  Future<bool> _handleFaceScan() async {
    ...
    // A large library scans for many minutes in this isolate; without a
    // heartbeat the lock goes stale after 20 min and the foreground scan
    // legally takes it, starting a second detector alongside this one.
  ...
  Future<bool> _runBackup(
    ...
    try {
      ...
      // The scan:false entry point (_handleUploadWorker) never calls
      // scanAndEnqueue, which is where _ready is awaited internally. Without
      // this, a queue still being restored from disk reads as empty and the
      // whole run exits as "nothing queued" before a single upload.
      await engine.ready;
