...
class ManifestService {
  ...
  /// Load the previously persisted manifest, restoring the sync baseline.
  ///
  /// `FileManifestStore.save` is called on every successful sync, but without
  /// this the manifest and — critically — [_partitionHashes] start empty on
  /// every cold start. An empty baseline means `getDirtyPartitionIds` reports
  /// *every* partition dirty, so each launch re-uploads the user's entire
  /// metadata history instead of only what changed. `setManifest` is what
  /// repopulates the baseline, so loading through it keeps one code path.
  ///
  ...
  Future<void> initialize() async {
    ...
    if (persisted != null) setManifest(persisted);
  }
