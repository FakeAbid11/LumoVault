...
class _ErrorBoundaryState extends State<ErrorBoundary> {
  Object? _error;
  Widget Function(FlutterErrorDetails)? _previousHandler;

  @override
  void initState() {
    super.initState();
    // Intercept rendering errors from child widgets so they don't crash the
    // whole app — instead they land here and show the fallback error UI.
    _previousHandler = ErrorWidget.builder;
    ErrorWidget.builder = _handleFlutterError;
  }

  @override
  void dispose() {
    // Restore the original handler so we don't leak into sibling boundaries.
    if (_previousHandler != null) {
      ErrorWidget.builder = _previousHandler!;
    }
    super.dispose();
  }

  Widget _handleFlutterError(FlutterErrorDetails details) {
    if (mounted) {
      // Schedule the state update after the current frame to avoid calling
      // setState during build (which Flutter forbids).
