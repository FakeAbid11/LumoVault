...
/// load and its score threshold. SCRFD tiers share strides [8, 16, 32] and
/// 1–2 anchors per cell, so the decode math is identical; only the asset and
/// threshold differ.
...
/// Matches the per-face crop files written by [FaceDetectionService] into the
/// OS temp directory (`face_<millis>_<seq>.jpg`).
final RegExp faceCropFilePattern = RegExp(r'^face_\d+_\d+\.jpg$');

/// Captures the creation millis from a crop file name (see
/// [faceCropFilePattern]) so eviction can order by real insertion time.
final RegExp _faceCropMillisPattern = RegExp(r'^face_(\d+)_\d+\.jpg$');

/// Largest total size the face-crop cache may occupy before the oldest crops
/// are trimmed.
const int maxFaceCropCacheBytes = 100 * 1024 * 1024;

/// Deletes the oldest face crops until their total size is back under
/// [maxBytes], returning how many files were removed.
///
/// Crops live in the OS temp root referenced by absolute path from the
/// database, so they sit OUTSIDE [ThumbnailCache]'s bounded directory and
/// nothing trimmed them: at roughly 10-20 KB per face a 10k-face library grew
/// past 200 MB on devices with 6-8 GB of RAM.
///
/// A trimmed person tile falls back to its letter avatar — the same
/// degradation the OS causes whenever it purges this directory under storage
/// pressure, since that is where these files live. The DB rows are left
/// alone, so a face rescan restores the crop.
///
/// [directoryOverride] exists for tests; production resolves the OS temp dir.
Future<int> evictFaceCropCache({
  int maxBytes = maxFaceCropCacheBytes,
  @visibleForTesting Directory? directoryOverride,
}) async {
  final dir =
      directoryOverride ??
      await getTemporaryDirectory().catchError((Object e) {
        debugPrint(
          '[FaceCropCache] Temp dir unavailable, skipping eviction: $e',
        );
        return Directory.systemTemp;
      });
  if (!await dir.exists()) return 0;

  final crops = <File, int>{};
  int total = 0;
  try {
    await for (final entity in dir.list()) {
      if (entity is! File) continue;
      if (!faceCropFilePattern.hasMatch(p.basename(entity.path))) continue;
      final stat = await entity.stat();
      crops[entity] = stat.size;
      total += stat.size;
    }
  } catch (e) {
    debugPrint('[FaceCropCache] Listing failed, skipping eviction: $e');
    return 0;
  }

  if (total <= maxBytes) return 0;

  // Oldest first. The name carries the creation millis, so parse it as an
  // INT — comparing the names as strings would order a 13-digit timestamp
  // before a 14-digit one and evict the wrong files.
  final byAge = crops.keys.toList()
    ..sort((a, b) => _cropAge(a).compareTo(_cropAge(b)));

  var deleted = 0;
  for (final file in byAge) {
    ...
    try {
      ...
      // Another writer may have raced us; the next sweep catches up.
    }
  }

  if (deleted > 0) {
    debugPrint('[FaceCropCache] Trimmed $deleted face crop(s)');
  }
  return deleted;
}

/// Creation time embedded in a crop file name; 0 for an unparseable one, which
/// makes it sort oldest-first (it is the least trustworthy, so it goes).
int _cropAge(File file) =>
    int.tryParse(
      _faceCropMillisPattern.firstMatch(p.basename(file.path))?.group(1) ?? '',
    ) ??
    0;
...
class FaceDetectionService {
  ...
  /// In-flight [_init] call. The constructor kicks one off, and
  /// [ensureInitialized] and [detectFacesWithThumbnails] can both reach a
  /// not-yet-initialized service concurrently — without this each ran the
  /// full init and double-created the ONNX sessions and the worker isolate.
  /// Cleared in [_initNow]'s finally so a failed init can be retried.
  Future<void>? _initInFlight;

  /// Per-session readiness. [_initialized] only flips once BOTH sessions have
  /// loaded, so gating disposal on it skipped the close entirely when init
  /// failed partway (detector up, embedder failed) and leaked the detector
  /// session. These track each session on its own.
  bool _detectorSessionReady = false;
  bool _embedderSessionReady = false;

  /// The one embedder accepted since v20. The former silent fallback to
  /// w600k_mbf mixed two incomparable 512-d vector spaces — faces from one
  /// model satisfied the other's centroids and different people merged into
  /// one group.
  static const String edgefaceEmbedderAsset =
      'assets/models/edgeface_xs_gamma_06.onnx';

  String _activeEmbedderAsset = edgefaceEmbedderAsset;

  /// Model tag stamped onto every face row (`Faces.embeddingModel`) and the
  /// person centroid (`People.centroidModel`) — clustering refuses vectors
  /// whose tags differ.
  String get embedderModelTag => _activeEmbedderAsset.split('/').last;
  ...
  Future<void> _initNow() async {
    try {
      ...
      // There used to be a tier-selected detector with a silent fallback to
      // this asset; it was removed because the tier's asset was a truncated
      // placeholder (see faceDetectionServiceProvider), which made every
      // high-core device fail over to 500M unnoticed. An inability to load
      // the configured detector is now a hard init failure that surfaces.
      _detectorSession = await _ort.createSessionFromAsset(
        config.detectorAsset,
        options: threadOptions,
      );
      _detectorSessionReady = true;
      // EdgeFace-XS-GAMMA is the ONLY accepted embedder. Vectors from two
      // embedding models are not comparable, and the old silent
      // w600k_mbf fallback let faces from different spaces merge people
      // across models. If it cannot load, init fails — detectFacesWith-
      // Thumbnails then throws FaceDetectorUnavailable and the scan surfaces
      // a visible error instead of quietly switching vector spaces.
      _embedderSession = await _ort.createSessionFromAsset(
        edgefaceEmbedderAsset,
        options: threadOptions,
      );
      _embedderSessionReady = true;
      _activeEmbedderAsset = edgefaceEmbedderAsset;
      ...
      _initialized = true;
      debugPrint(
        '[FaceDetectionService] ONNX models + worker isolate ready (embedder: $_activeEmbedderAsset)',
      );
    } catch (e) {
  ...
  Future<FaceDetectionResult> detectFacesWithThumbnails(
    ...
    try {
      ...
      if (firstResult == null || firstResult.imgWidth == 0) {
        // A null or zero-dimension reply means the worker died mid-call: a
        // dead port answers null outright, and a timed-out call respawns the
        // worker and still returns null. Returning FaceDetectionResult.empty()
        // here used to record "no faces found" in the scan log, so every photo
        // processed after the worker died was permanently poisoned as empty —
        // the exact failure FaceDetectorUnavailable exists to prevent.
        throw const FaceDetectorUnavailable(
          'Preprocessing worker returned no image',
        );
      }
      ...
      if (faceResult == null) {
        throw const FaceDetectorUnavailable(
          'Preprocessing worker returned no face crops',
        );
      }
  ...
  Future<void> dispose() async {
    ...
    _workerExitPort?.close();
    _workerExitPort = null;
    // Close whichever sessions actually loaded. _initialized only flips once
    // BOTH come up, so gating here on it skipped the close entirely when init
    // failed partway (detector up, embedder failed) and leaked the detector
    // session; the late fields would throw if closed unassigned, so the
    // per-session flags are also the null-safety guard.
    if (_detectorSessionReady) {
      _detectorSessionReady = false;
      await _detectorSession.close();
    }
    if (_embedderSessionReady) {
      _embedderSessionReady = false;
