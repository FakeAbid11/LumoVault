...
class FaceDetectionService {
  ...
  /// Which embedder model is active — EdgeFace-XS when available, w600k_mbf otherwise.
  String _activeEmbedderAsset = 'assets/models/w600k_mbf.onnx';
  ...
  Future<void> _init() async {
    try {
      ...
      // Try EdgeFace-XS first (smaller, faster, higher accuracy), fall back to w600k_mbf.
      try {
        _embedderSession = await _ort.createSessionFromAsset(
          'assets/models/edgeface_xs_gamma_06.onnx',
          options: threadOptions,
        );
        _activeEmbedderAsset = 'assets/models/edgeface_xs_gamma_06.onnx';
        debugPrint('[FaceDetectionService] Using EdgeFace-XS embedder');
      } catch (e) {
        debugPrint(
          '[FaceDetectionService] EdgeFace-XS not bundled — falling back to w600k_mbf: $e',
        );
        _embedderSession = await _ort.createSessionFromAsset(
          'assets/models/w600k_mbf.onnx',
          options: threadOptions,
        );
        _activeEmbedderAsset = 'assets/models/w600k_mbf.onnx';
      }
