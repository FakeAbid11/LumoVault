...
void main() {
  ...
  testWidgets('the info action opens the backup detail sheet', (tester) async {
    ...
    // Markers unique to TelegramItemDetailSheet. This assertion used to be
    // `find.text('Backed up to Telegram')` + the file name, which ExifDetailsSheet
    // *also* renders in its backup block — so it passed green while the button
    // opened a different widget and the detail sheet stayed unreachable dead
    // code.
    expect(find.text('Camera & EXIF details'), findsOneWidget);
    expect(find.text('File name'), findsOneWidget);
    expect(find.byType(ExifDetailsSheet), findsNothing);
  });

  testWidgets('the detail sheet hands off to the EXIF sheet', (tester) async {
    final downloads = _HoldingDownloadService();
    ...
    await tester.tap(find.text('Camera & EXIF details'));
    await tester.pumpAndSettle();

    // The backup sheet is replaced by the EXIF sheet rather than merely
    // dismissed, so cloud items keep a working route to camera details.
    expect(find.text('Camera & EXIF details'), findsNothing);
    expect(find.byType(ExifDetailsSheet), findsOneWidget);
  });
}
