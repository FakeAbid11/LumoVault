...
///
/// Throwing from the handler re-queues the batch so the next [
/// SyncService.enqueueChange] re-arms the debounce and retries it.
...
class SyncService {
  ...
  /// On failure the batch is re-queued and the next [enqueueChange] re-arms
  /// the debounce — no self-retry, so a persistently failing handler can't
  /// spin.
  Future<void> _processPendingChanges() async {
    ...
    try {
      ...
      _logSync(
        ...
        success: false,
        error: e.toString(),
        details: '${batch.length} change(s) re-queued',
      );
      await _persistLog();
    }
  }
