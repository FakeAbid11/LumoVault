...
class FaceDao extends DatabaseAccessor<AppDatabase> with _$FaceDaoMixin {
  ...
  ///
  /// Transactional: `faces.person_id` has no enforced foreign key, so a crash
  /// between the two statements left faces pointing at a deleted person.
  Future<void> deletePerson(int personId) => transaction(() async {
    await (update(faces)..where((t) => t.personId.equals(personId))).write(
      const FacesCompanion(personId: Value(null)),
    );
  ...
  ///
  /// Transactional for the same reason as [deletePerson] — a half-applied merge
  /// would strand the source's faces on a person id that no longer resolves.
  ...
  /// Whether every visible item has a row in the scan log.
  ///
  /// KNOWN DEFECT, left as-is deliberately: comparing COUNT(face_scans) to
  /// COUNT(media_items) drifts upward, because MediaDao deletes media rows
  /// without any matching pruning of face_scans — this DAO has no per-item
  /// scan-log delete at all — so the numerator only grows while the denominator
  /// shrinks and this can stay true while new photos still await scanning.
  /// Fixing it properly needs either a scan-log prune wired into media
  /// deletion, widening this accessor's `@DriftAccessor(tables: ...)` to reach
  /// media_items, or a real `ON DELETE CASCADE` in a migration. Those are
  /// schema-scope decisions, so it is flagged rather than half-done here.
  ...
  Future<void> clearForRescan() => transaction(() async {
