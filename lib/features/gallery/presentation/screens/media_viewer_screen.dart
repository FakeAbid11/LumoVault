...
class _MediaViewerScreenState extends ConsumerState<MediaViewerScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    // Live queue status for this asset, so the button reflects a backup already
    // running for it (started here, from a tile, or by a full backup run).
    //
    // Only the STATUS is watched here, via select, and only so this screen
    // rebuilds when a backup starts/finishes (to show or drop the button). The
    // underlying provider recomputes on every throttled stats tick, and
    // watching it whole used to rebuild the ENTIRE viewer — PageView, gradient
    // stack, action bar — ~2x a second for the whole duration of an upload. The
    // per-tick progress value is consumed by [_ViewerBackupButton], which scopes
    // those rebuilds to the one button.
    final taskStatus = ref.watch(
      uploadTaskForItemProvider(asset.id).select((t) => t?.status),
    );
    // Hide the backup button the moment its task reports completion, even if
    // the gallery record's status flip lands on the same tick. This is also
    // what the taskStatus watch above exists to react to.
    final isBackedUpNow = isBackedUp || taskStatus == UploadStatus.completed;
                          ...
                          child: Padding(
                            ...
                            child: Row(
                              children: [
                                if (!isBackedUpNow)
                                  Expanded(
                                    child: _ViewerBackupButton(
                                      mediaItemId: asset.id,
                                      isInFlight: _inFlight.contains(asset.id),
                                      onBackup: () => _backUpCurrentAsset(),
                                    ),
                                  ),
                                Expanded(
                                  child: _BottomAction(
  ...
  void _openTagEditor() {
    ...
    showModalBottomSheet(
      ...
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (context) => TagEditorSheet(mediaId: asset.id, asset: asset),
    );
  }
  ...
  Future<void> _trashFromDevice() async {
    ...
    try {
      ...
      await repository.moveToTrash(asset.id, asset: asset);
...
/// Backup button that owns its live-progress watch.
///
/// The [MediaViewerScreen] build deliberately does NOT watch the full task
/// (only a status select), so a background upload's ~2 Hz stats ticks can't
/// rebuild the whole viewer; this small leaf rebuilds twice a second instead,
/// which is exactly where the progress ring needs it.
class _ViewerBackupButton extends ConsumerWidget {
  const _ViewerBackupButton({
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final task = ref.watch(uploadTaskForItemProvider(mediaItemId));
    final isUploading = isInFlight || task?.status == UploadStatus.uploading;
    return _BackupAction(
      isUploading: isUploading,
      isQueued: task?.status == UploadStatus.queued,
      progress: task?.progress ?? 0,
      onPressed: isUploading ? null : onBackup,
    );
  }
}
