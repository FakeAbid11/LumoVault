...
class TelegramDownloadService implements DownloadService {
  ...
  Future<DownloadResult> downloadFile({
    ...
    try {
      ...
      subscription = _updates.listen((update) {
        ...
        // dispose() closes the controller during teardown; a TDLib update
        // that lands in that window would otherwise throw on a closed stream.
  ...
  /// Releases the resources this service holds for its lifetime.
  ///
  /// In-flight downloads fail exactly as [cancelDownload] fails them — their
  /// completers resolve to a 'Download cancelled' error rather than hanging
  /// on a stream nobody listens to anymore — and the progress stream is
  /// closed. Idempotent: safe to call more than once.
  void dispose() {
