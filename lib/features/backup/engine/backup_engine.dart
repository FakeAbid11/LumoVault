...
class BackupEngine {
  ...
  /// Set once [_resolveChannelId] has confirmed the storage channel through
  /// [StorageChannelService.findOrCreateChannel], which does its own presence
  /// probe. Keeps that probe to once per engine rather than once per upload.
  ...
  /// Pending task count read straight from the live queue.
  ///
  /// [stats] is throttled by up to `_statsThrottleDuration` and only refreshed
  /// when something calls `_updateStats()`, so it must not be used to decide
  /// "is there work right now". The background runner's early-exit gate and the
  /// scanner's re-enqueue check both read [stats] and therefore usually saw 0
  /// immediately after a scan that had just queued hundreds of items.
  ///
  /// Await [ready] before trusting a zero: the queue is empty until the
  /// persisted snapshot has been merged back in.
  int get queuedPendingCount => _queue.pendingCount;
  ...
  Future<void> _uploadTask(UploadTask task) async {
    ...
    try {
      ...
      if (shouldRetry) {
        ...
        // Stamped through _updateTaskIfQueued rather than updateTask: this used
        // to read `getTaskById(task.id) ?? task`, so a task the user cancelled
        // mid-batch fell back to the stale snapshot and updateTask's
        // unknown-id re-add put it straight back in the queue — resurrecting
        // exactly what cancelTask had just removed.
        _updateTaskIfQueued(task.id, (t) => t.copyWith(nextAttemptAt: wakeAt));
        debugPrint(
  ...
  Future<int> _resolveChannelId() async {
    ...
    // `_channelVerified` keeps this to one presence probe per engine
    // lifetime. This used to return the cached id unconditionally, so a
    // channel the user deleted from the Telegram app produced permanent
    // failures forever — findOrCreateChannel's own revalidation was never
    // reached from the upload path at all.
    if (cached != null && _channelVerified) return cached;
