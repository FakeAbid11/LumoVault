...
class GalleryRepository {
  ...
  /// Index of [localId] in the read model, building its record on demand from
  /// [asset] when it has never been scanned. Returns -1 when there is nothing
  /// to act on.
  ///
  /// Mutating methods that only had a `if (_indexOfLocalId(id) != -1)` guard
  /// used to return silently for an unscanned item, so the user's edit looked
  /// applied in the UI and was never written anywhere. Callers that can supply
  /// [asset] should, so the write lands.
  Future<int> _indexOrCreate(String localId, AssetEntity? asset) async {
    final index = _indexOfLocalId(localId);
    if (index != -1) return index;
    if (asset == null) return -1;
    if (await ensureMediaItemExists(asset) == null) return -1;
    return _indexOfLocalId(localId);
  }
  ...
  ///
  /// Pass [asset] when the item may never have been scanned, otherwise the
  /// write is dropped: it used to guard on the in-memory index alone, so
  /// tagging an unscanned photo showed the new chips and saved nothing.
  Future<void> setTags(
    String localId,
    List<String> tags, {
    AssetEntity? asset,
  }) async {
    final index = await _indexOrCreate(localId, asset);
    if (index == -1) return;
  ...
  ///
  /// Pass [asset] when the item may never have been scanned, or the deletion
  /// is dropped — see [setTags] for the same guard and why it needs the asset.
  Future<void> moveToTrash(String localId, {AssetEntity? asset}) async {
    final index = await _indexOrCreate(localId, asset);
    if (index == -1) return;
    final updated = _mediaItems[index].copyWith(
      isTrashed: true,
    ...
    _mediaItems[index] = updated;
    await _persistItem(updated);
    _notifyMetadataChange(localId: localId, operation: 'trash', item: updated);
  }
