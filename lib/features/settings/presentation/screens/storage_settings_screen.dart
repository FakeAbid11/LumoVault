...
class StorageSettingsScreen extends ConsumerWidget {
  ...
  Future<void> _repairMetadata(BuildContext context, WidgetRef ref) async {
    ...
    try {
      ...
      // self-correct on the next scan or thumbnail rebuild. The message says
      // so rather than claiming repairs this screen never performs.
        ...
        SnackBar(
          content: Text(
            result.issues.isEmpty
                ...
                : 'Metadata check: ${result.issues.length} issue(s) found. '
                      '${result.fixableCount} will self-correct on the next '
                      'scan; ${result.issues.length - result.fixableCount} '
                      'need attention.',
          ),
        ),
      );
