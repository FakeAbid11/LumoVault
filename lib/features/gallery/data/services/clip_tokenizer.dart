...
class ClipTokenizer {
  ...
  factory ClipTokenizer.fromLines(
    ...
    return ClipTokenizer._(encoder, bpeRanks, byteEncoder, contextLength);
  }
  ...
  /// CLIP's byte→unicode table, kept past the vocab build: [tokenize] maps
  /// each token's UTF-8 bytes through it before BPE, exactly like OpenCLIP's
  /// `''.join(byte_encoder[b] for b in token.encode('utf-8'))`.
  ...
  List<int> tokenize(String text) {
    ...
    for (final match in _tokenPattern.allMatches(cleaned)) {
      // Byte-encode before BPE, as OpenCLIP does. Without this step a token
      // beyond latin-1 (Cyrillic, CJK, emoji) is looked up raw, is absent
      // from the vocab, and is silently dropped by the null check below —
      ...
      final byteToken = utf8.encode(token)
          ...
          .whereType<String>()
          .join();
      for (final piece in _bpe(byteToken).split(' ')) {
