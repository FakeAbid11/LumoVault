...
/// The memory cache is a true LRU (reads re-order it). The disk cache is
/// trimmed by [evictIfOverSize], which orders by file *modification* time —
/// closest to FIFO, since a thumbnail is written once and reads don't touch
/// the file — and now runs from [put] as well as from the periodic rebuild
/// task, so the directory can no longer drift past its budget between
/// rebuilds.
class ThumbnailCache {
  ...
  /// Disk eviction is a full directory listing plus a stat per file, so it
  /// must not run on every write — a full-library scan does one write per
  /// photo. Every Nth [put] checks the budget instead.
  static const int _evictionCheckInterval = 64;
  int _putsSinceEvictionCheck = 0;
  bool _evictionRunning = false;
  ...
  ///
  /// No-ops (memory-cache only) when [initialize] hasn't run yet, instead of
  /// crashing on a null cache directory.
  Future<void> put(String mediaItemId, Uint8List bytes) async {
    ...
    // Trim the disk budget from the write path too. Without this, eviction
    // only ran from the periodic thumbnail-rebuild task, so between rebuilds
    // the directory grew past its size budget unbounded — a full-library
    // scan could blow well beyond the intended ceiling.
    if (++_putsSinceEvictionCheck >= _evictionCheckInterval) {
      _putsSinceEvictionCheck = 0;
      // Fire-and-forget: the write the caller asked for is already done, and
      // making every tile wait on a directory sweep would dominate scan time.
      unawaited(evictIfOverSize());
    }
  }
  ...
  ///
  /// Re-entrant calls are collapsed: [put] fires this off periodically while
  /// the periodic rebuild task also calls it, and two sweeps deleting the same
  /// file would make the loser throw.
  Future<void> evictIfOverSize({int? maxSizeBytes}) async {
    if (_evictionRunning) return;
    _evictionRunning = true;
    try {
      await _evictIfOverSizeInner(maxSizeBytes);
    } finally {
      _evictionRunning = false;
    }
  }

  Future<void> _evictIfOverSizeInner(int? maxSizeBytes) async {
    ...
    // Oldest modification first. This is FIFO rather than true LRU: reads
    // never touch the file, so mtime tracks insertion, not use. See the
    // class doc.
    ...
    for (final entry in files) {
      ...
      // Also remove from memory cache. `_fileFor` appends exactly one `.jpg`,
      // so stripping the last extension recovers the key even if an id ever
      // contains a dot.
      final name = p.basenameWithoutExtension(entry.file.path);
