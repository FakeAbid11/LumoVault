...
class UploadQueue {
  ...
  /// Live counts per status, maintained incrementally to avoid full-queue scans.
  final Map<UploadStatus, int> _counts = {};

  void _incrementCount(UploadStatus status) {
    _counts[status] = (_counts[status] ?? 0) + 1;
  }

  void _decrementCount(UploadStatus status) {
    final current = _counts[status];
    if (current != null) {
      if (current <= 1) {
        _counts.remove(status);
      } else {
        _counts[status] = current - 1;
      }
    }
  }
  ...
  int get pendingCount => _counts[UploadStatus.queued] ?? 0;
  int get uploadingCount => _counts[UploadStatus.uploading] ?? 0;
  int get completedCount => _counts[UploadStatus.completed] ?? 0;
  int get failedCount => _counts[UploadStatus.failed] ?? 0;
