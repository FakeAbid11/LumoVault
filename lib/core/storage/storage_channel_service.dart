...
import '../../features/metadata/data/models/manifest.dart';
...
/// Outcome of probing whether a chat id still resolves.
///
/// [unknown] is kept separate from [absent] on purpose: "TDLib couldn't answer"
/// must not be acted on the same way as "Telegram says this channel is gone",
/// because the recovery for the latter can create a second storage channel.
enum _ChatPresence { present, absent, unknown }
...
class StorageChannelService {
  ...
  Future<StorageChannelResult> findExistingChannel() async {
    if (await _cachedChannelIsUsable()) {
      return StorageChannelFound(channelId: _cachedChannelId!);
    }
  ...
  /// Whether the cached channel id is still usable, per a live probe.
  ///
  /// An *inconclusive* probe counts as usable. Falling through to the
  /// search-and-maybe-create path because TDLib happened to be momentarily
  /// unreachable is precisely how a user's backup ends up split across two
  /// channels, and reusing a live id costs nothing. Only a definitive "no such
  /// chat" should send us searching. The old `_chatExists` returned false on
  /// any error, which quietly made a transient failure look like a deletion.
  Future<bool> _cachedChannelIsUsable() async {
    final cached = _cachedChannelId;
    if (cached == null) return false;
    if (await _probeChat(cached) == _ChatPresence.absent) {
      debugPrint('[StorageChannel] Cached channel $cached is gone, searching');
      return false;
    }
    return true;
  }

  /// Result of asking TDLib whether a chat still exists.
  Future<_ChatPresence> _probeChat(int chatId) async {
    try {
      ...
      if (chatInfo['@type'] == 'error') {
        final detail = '${chatInfo['message']}';
        const goneIndicators = [
          'CHAT_INVALID',
          'CHAT_NOT_FOUND',
          'CHAT_DELETED',
        ];
        return goneIndicators.any(detail.contains)
            ? _ChatPresence.absent
            : _ChatPresence.unknown;
      }
      return chatInfo['id'] != null
          ? _ChatPresence.present
          : _ChatPresence.unknown;
    } catch (e) {
      // Transport failure, TDLib still connecting, timeout — none of these say
      // anything about whether the channel exists.
      debugPrint(
        '[StorageChannel] Presence probe for $chatId inconclusive: $e',
      );
      return _ChatPresence.unknown;
    }
  }
  ...
  ///
  /// This is a point-in-time record taken when the channel is empty, and it is
  /// never rewritten — the live manifest is the document that
  /// TelegramMetadataUploader posts on every sync. Read it only as a legacy
  /// fallback for a backup predating that document.
  Future<void> _pinManifestMessage(int channelId) async {
    final manifest = {
      'app': 'lumovault',
      // Was a hardcoded 1, which made every restore's compatibility check pass
      // against a constant instead of the version that really wrote the data.
